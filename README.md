...
**`(New)`로 시작하는 피그마 프레임만 번역 대상이며, `Case`로 시작하는 피그마 프레임은 위키에 업데이트하지 않습니다.** 각 필터는 해당 단계에서만 적용됩니다 — 다른 단계에 적용하면 화면·인터랙션 누락이 발생합니다.

| 단계 | 화면(프레임) 범위 |
|------|------------------|
| 1단계 번역 | 이름이 `(New)`로 시작하는 프레임**만** 텍스트 추출·번역 |
| 2단계 프로토타입 | 페이지 직속 자식 프레임 **전체** + 모든 인터랙션 (이름·크기 필터 금지) |
| 3단계 위키 | 이름이 `Case`로 시작하는 프레임 **제외**, 나머지 전체 (단, `(New) Case2...`처럼 `(New)`로 시작하면 포함) |

또한 화면별 Figma 코멘트는 위키 Screen 표의 Description에 포함하며, 화면 상단(y좌표 기준) 코멘트부터 `1.`, `2.`, … 순차 번호를 붙입니다.
