...
> 담당자: `hogeun` · 마지막 갱신: 2026-09-23 · 세션 #1(Claude 참여 기준) · 위키 **v16**
>
...
- [ ] 🔴 **P0 — `category_promotion_banner`에 필드 3개 추가**(FE·BE): ⛔ **신규 컬렉션 없음**(임시 운영). `maintenance`(object — `bannerText`·`ctaText`·`ctaButtonLabel`) + `maintenance_banner_on_off`(② 배너) + `maintenance_buy_block_on_off`(③ 구매 차단). 기존 `contents`는 **카테고리 전용** 유지 (위키 5-5)
- [x] ~~🔴 P0 — ② 사전 고지 가능 여부~~ → **해소(2026-09-23)**. **스위치 2개로 분리 확정** — 배너는 D-7부터, 구매 차단은 D-Day 00:00부터 따로 움직인다
- [ ] 🔴 **P0 — ① `contents.title` 원문 백업**: 점검 중 `maintenance.bannerText`를 복사해 덮고 종료 후 복구한다. **LPC에 리비전 이력이 없어** 백업이 유일한 복구원 — 5개 언어 현재값을 저장소에 남긴다
...
| 2026-09-23 | ⛔ **신규 컬렉션 대신 기존 `category_promotion_banner`에 필드 2개 추가** — `maintenance` + `maintenance_on_off` | 사용자 결정(**임시 운영**). `contents`는 카테고리 전용으로 유지하고 ①은 점검 문구를 **복사해 적용** 후 원문 복구. 종전 `maintenance_notice` 신설안·XLT 2키안은 **모두 철회** |
| 2026-09-23 | ⛔ **스위치는 2개로 분리** — `maintenance_banner_on_off`(② 배너) · `maintenance_buy_block_on_off`(③ 구매 차단) | 사용자 결정. 켜는 시점이 **D-7 ↔ D-Day 00:00**으로 달라 하나로 묶으면 배너를 띄우는 순간 **구매가 7일 일찍 막힌다**. 이름은 둘 다 `_on_off`로 **극성 통일**(`true`=동작 켬) — `_disabled` 혼용 시 `false`가 이중부정이 된다 |
| 2026-09-23 | **On/Off는 문구 필드 안이 아니라 별도 boolean 필드** | ⓐ 이 컬렉션의 기존 관례(`on_off`가 `contents`와 분리) ⓑ CMS에서 boolean은 토글 — JSON 안이면 문구를 직접 편집해야 하고 **전체 교체 PUT**이라 오타가 필드를 날린다(2026-09-14 사고) ⓒ FE 분기 한 줄. ⚠️ `on_off`가 **로케일 5벌**인 것은 실측 확인 — 어느 안이든 같아 판단 근거가 아니다 |
| 2026-09-23 | **① 카테고리 배너는 점검 문구를 복사해 쓴다**(override 자동화 아님) | 사용자 결정. `maintenance.bannerText` → `contents.title` 수동 복사 · 종료 후 원문 복구(5개 언어) |
