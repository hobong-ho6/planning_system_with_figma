...
> 담당자: `hogeun` · 마지막 갱신: 2026-09-23 · 세션 #1(Claude 참여 기준) · 위키 **v13**
>
...
- [ ] 🔴 **P0 — ②·③ 구현 구조 승인**: ③에 구조 제안을 실었다(위키 5-5) — **XLT 2키**(`mini_voucher_detail_buy_now_btn_disabled` · `mini_voucher_detail_maintenance_desc` · LV·Dapp Portal·`{0}`) + **LPC 신규 단건 컬렉션 `maintenance_notice`**(`on_off` + `schedule.starts_at/ends_at` + `contents.datetime_label`). **② 도 같은 방식으로** 가면 종전 「`voucher_product` uid별 배너 필드 신설」이 불필요해진다(올리브영 영향 🟢로 브랜드 분기 소멸). FE·기획 승인 필요
- [ ] **P1 — 신규 XLT 키 확정 전 중복 대조**: `fetch_xlt_registry.py --similar` + `check_pending_keys.py --wiki`(§14)
...
| 2026-09-23 | **③ 하단 CTA는 「XLT 문구 + LPC 스위치」 — ①의 LPC 배너 방식과 다르다** | ③은 **버튼 상태가 본체**라 문구 교체로 성립하지 않는다. 버튼 라벨이 이미 XLT(`mini_voucher_detail_buy_now_btn`)이고 비활성 라벨 교체 선례도 XLT(`purchase_drops_soldout_buy_btn`). 점검 일시는 변동값이라 **치환자 `{0}`** 로 빼고 LPC에서 주입 |
