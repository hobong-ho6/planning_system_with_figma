...
> 담당자: `hogeun` · 마지막 갱신: 2026-09-23 · 세션 #1(Claude 참여 기준) · 위키 **v15**
>
...
- [ ] 🔴 **P0 — `category_promotion_banner`에 필드 2개 추가**(FE·BE): ⛔ **신규 컬렉션 없음**(임시 운영). `maintenance`(object — `bannerText`·`ctaText`·`ctaButtonLabel`) + `maintenance_on_off`(boolean). `true`면 **② 상세 배너 노출 + ③ 구매 버튼 비활성**(스위치 1개가 두 동작). 기존 `contents`는 **카테고리 전용** 유지 (위키 5-5)
- [ ] 🔴 **P0 — ② 사전 고지 가능 여부**: 스위치가 하나라 D-7에 켜면 **구매도 7일 일찍 막힌다** → 현 설계상 ②는 **D-Day 00:00부터**만 뜬다. D-7~D-Day 상세 사전 고지가 필요하면 **스위치 분리**(`maintenance_banner_on_off` / `maintenance_buy_disabled`) 필요
