...
### 2026-09-03 — 세션 #4(GA): GA Event 정의 + FAQ `id` + LPC 문서 갱신 (v102→v106 · LPC v2→v4)

- 완료: `md/GA.md` 신설 후 이 위키로 첫 적용 — 헤더 `XLT & GA` · view 6 · click 18(기존 상세 Event 표 이름 보존·`#`만 44건 번호로 재매핑) · 배너·OA 제외 표기 · XLT No 정정 42행 · `click_faq`용 `landpress/faq_*.json` `id` 4건(CMS 반영 완료) · LPC 문서 §7 갱신. 페이지가 세션 중 v100→v102→v105로 계속 바뀌어 **PUT 직전 라이브 재조회 + 앵커 assert**로만 넣었다
