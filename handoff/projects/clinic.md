...
> 담당자: hogeun · 마지막 갱신: 2026-09-04 · 세션 #4 · 마지막 커밋 `fb778e0`
...
- 위키: `4667512757` 「클리닉 예약 동선 개선」 — **현재 v117** (부모 `3910828993`)
  - **하위 `4686692164` 「클리닉 LPC(Landpress) 관리 영역 정리」**(2026-09-03 신설 · **v13**) — LPC 영역 10곳의 컬렉션·필드·구조·**실등록 JSON** + 크롭 이미지 10장
  - **하위 `4685320765` 「클리닉 일본어 문구 검수 (日本語レビュー)」**(2026-09-04 신설 · v1) — XLT 87건 + Landpress 3필드 **ko↔ja 대조표**(수정안 기입란) · ja 미번역 목록 · 베타 `https://unifi-web.line-apps-beta.com/k-pick/clinic`
...
- [ ] 🔴 **P0 일본어 검수 요청(사용자→일본 현지 담당자)** — 하위 `4685320765`. 검수 완료 후 수정안을 부모 문서 다국어 표·Landpress JSON에 반영하고 XLT·CMS 업로드
- [ ] **P1 ja 미번역 3종** — `faq` 신규 5건 · `voucher`(title·moreLabel·상품명) · 병원별 3곳 × `bridge_info`·`menu`. 검수 페이지 3절에 「제안 기입란」으로 노출 — **번역이 먼저**이고 그 뒤 재검수
- [ ] **P1 Landpress ja `口コミ` 잔존** — `cautions_ja_JP.json` items[1]. XLT는 `レビュー`로 통일했으므로 **같은 화면에서 갈린다** — 검수 때 함께 정리 권장
