...
>
> 📌 **세션 #8 — 클리닉 LPC 정비 + Unifi 마스터 대조 + My 바우처 상세 신규 3키**. 가격 기본값을 XLT 등록값 `상담 문의`로 통일(46건×5언어)하고 **병원 21곳 · beta·prod 전건 동치**(ACTIVE 22곳 중 **르아델은 사용자 지시로 제외**). Unifi 병원·바우처 마스터를 처음 대조했다. 위키 3건 갱신(본문 **v184** · LPC **v45** · 의료문구 **v7**). ⛔ **`k_pick_shopping_product`는 관리 대상이 아님**(2026-09-14 사용자 확정) — 관련 미결 3건 종결.
>
...
- [ ] 🔴 **P0 — 클리닉 한정해제 요건 ②③④ · 값 미확보 8개 파라미터** — 요건 ①만 충족이고 **①~④ 모두 충족돼야 성립** → 상세에 **시술명 노출 불가**. 8개 표(파라미터·현재값·담당·요건)와 결정 2건(㉮ `price` 3택 · ㉯ `deviceNotice` 리쥬란 포함 여부 법무)은 **LPC 위키 `4727978725` 6번 3항이 정본**. ⚠️ 2026-09-14 실측 — **Unifi 마스터에도 시술별 가격이 없다**(`priceItems` 병원당 1건 · 26건 중 24건 `price:"0"`) → 선택지 ①(시작가)은 **마스터 데이터 구조 변경이 선행**돼야 한다
...
- [x] ~~**P2 — 르아델(Leadel) 시술 항목 확정**~~ — ⛔ **2026-09-14 사용자 지시로 LPC에서 제거**(beta·prod 5개 언어 · 병원 22→21곳 · 전건 동치 재확인). 마스터 `priceItems`가 빈 배열이라 시술 항목을 만들 수 없었던 것이 사유다. ⚠️ 따라서 **「마스터 ACTIVE = LPC 등재」가 항상 성립하지는 않는다** — 시술 데이터가 없는 병원은 제외한다
- [ ] **P2 — 병원 식별자 체계 통일**: 21곳 중 **18곳 `uid`(문자열) / 3곳 `unifiId`(UUID · 슈가성형외과·디에이피부과 도산·브로우드)** — 이 3곳은 마스터 `externalId`가 `null`이다. 전역 규칙이 「beta↔prod 매핑은 `uid` 기준」이라 **uid 없는 3곳은 그 규칙으로 매핑 불가** — FE·기획 확인
...
- **`k_pick_shopping_product`는 관리 대상이 아니다**(2026-09-14) — LPC 위키 1번 표의 8개 컬렉션만 관리한다. 캐시백률·판매처 표기·beta/prod 분기·매장명 번역 차이는 전부 범위 밖
