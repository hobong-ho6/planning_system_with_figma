...
- **「100엔딜 프로모션」은 이 작업이 아니다** — 같은 날(2026-09-15) **병렬 세션**이 Figma SECTION `백엔딜 프로모션`(`74343:1703` · `Home - 100엔딜`·`100앤딜 상세`·`결제하기` 3프레임)의 XLT 13키를 처리했다(`cfaf682` · `reports/gate/gate_report_100yen_deal_2026-09-15.md`). 이 프로젝트는 SECTION `Home`(`74343:3254`)의 **1,000엔 바우처 배너**다. 키도 겹치지 않는다(`mini_home_voucher_banner_*` ↔ `mini_home_100yen_more`).
- ⚠️ **사용자의 최초 지시 제목이 「100엔샵 배너」였다** — Figma 내용(1,000엔 바우처)과 어긋나 지적 후 「1,000엔 바우처 배너」로 변경됐다. 「100엔딜」과 뒤섞이기 쉬운 명칭이니 **이후 대화에서 「100엔」이 나오면 어느 건인지 먼저 확인**한다.
