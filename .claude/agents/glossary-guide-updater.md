...
§5-1은 "용어집 버전이 오르면 가이드 zip 갱신은 **제안이 아니라 필수 동반**"이라고 규정하는데도, 2026-07-30 실측에서 **가이드가 v3.5, 라이브 용어집이 v3.9 — 4개 버전이 밀려 있었다.** 갱신 대상이 4곳으로 흩어져 있고 전체 용어 표는 100행+ 재생성이라 메인 작업 흐름에서 반복적으로 밀린 것이다. 이 누락을 없애는 것이 네 존재 이유다.
