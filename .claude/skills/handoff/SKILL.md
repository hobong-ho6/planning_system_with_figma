...
3. **온보딩 상태 확인** — 훅이 주입한 `⑤ 온보딩 상태`의 **조치만** 수행한다(아래 「온보딩」). 훅이 없으면 직접 판별한다.
...
## 온보딩 — 자동 판별하고, 못 알아내는 것만 묻는다

훅이 세션 시작 때 `⑤ 온보딩 상태`를 A~E로 판별해 주입한다. **판별로 알 수 있는 것은 절대 묻지 않는다.**

| 상태 | 판별 근거 | 사용자에게 물을 것 | 내가 할 것 |
|---|---|---|---|
| **A** 기존 사용자 · 등록된 PC | `handoff.person`(확정) 있고 `people/{키}.md`에 이 hostname이 있다 | **없음** | 바로 브리핑하고 착수 |
| **B** 기존 사용자 · 새 PC | `handoff.person`(확정)은 있는데 이 hostname이 파일에 없다 | **저장소 경로 1개** | `people/{키}.md` PC 표에 행 추가 · 세팅 열 ✅ |
| **C** 설정 누락 · 신원 확인 | `handoff.person`이 없지만 **이 hostname 또는 `user.email`이 어떤 `people/*.md`에 등록**돼 있다 | **「{확인된 사람} 님이신가요?」 1회** (PC 미등록이면 경로도) | ⛔ **새 people 파일을 만들지 않는다** — 빠진 건 `handoff.person` 한 줄뿐이다. 파일에서 확인된 값으로 **완성된 명령**을 안내 |
| **D** 신규 사용자 | 사람키 없고 hostname도 미등록 | **3개만** — ① 사람키(짧은 영문 소문자) ② 커밋에 쓸 이름·메일 ③ 담당할 프로젝트 | `people/{키}.md` 생성(`_TEMPLATE.md`) · 인덱스 담당자 갱신 · git config 안내 · 커밋 |
| **E** 사람키만 있음 | `handoff.person`(확정)은 있는데 그 이름의 파일이 없다 | **이 PC 경로 · 담당 프로젝트** | `people/{키}.md` 생성 후 위와 동일 |

- ⚠️ **`user.email`에서 딴 사람키는 「추정」이다** — `handoff.person`(확정)과 달리 실제 파일명과 어긋날 수 있다. 어긋나면 **추정 키로 새 파일을 만들지 말고**(사람 파일이 갈라진다) hostname·이메일 증거로 확인된 키를 쓴다. 훅이 증거와 근거를 함께 준다(2026-08-20 실측: 추정 키 `hogeun.kim.lnxt` vs 실제 `hogeun`).
- **판별 재료**는 훅이 함께 준다 — 등록된 사람 목록 · 담당자 없는 프로젝트 · 이 저장소 커밋 author 이력(사람 후보). C·D에서 후보를 제시할 때 쓴다.
- **git config는 사용자가 실행한다.** ⚠️ **예시 값을 넣은 실행 가능한 명령을 주지 않는다** — 2026-08-20에 `user.name "이름"`·`본인@example.com`이 그대로 실행돼 **5커밋이 잘못된 author로 공유 브랜치에 푸시**됐다. 값을 아는 경우(C처럼 파일에 있음)엔 그 값을 채워 완성된 명령으로 주고, 모르면 **먼저 묻는다.**
- 신규 사용자를 등록하면 `people/{키}.md` + 인덱스 담당자 갱신을 **같은 커밋으로 푸시**한다 — 로컬에만 있으면 다음 PC에서 없는 것과 같다.
...
- **`HANDOFF.md`**: 전역만. 인덱스는 **활성 프로젝트 1줄씩**. **20KB 이내**(줄이 아니라 `wc -c` 바이트로 잰다 — `handoff/README.md`가 정본).
  - ⛔ **여기에 남기는 건 셋뿐이다**(2026-09-22 압축으로 20.4KB→9.3KB): ① 활성 프로젝트 1줄 ② **모르면 사고 나는 차단 규칙 한 줄** ③ 어디를 열지(포인터). 근거·경위·절차는 **[`md/handoff-context.md`](../../../md/handoff-context.md)**(전역 실측 §1~16) · **[`md/decisions.md`](../../../md/decisions.md)**(결정 전문) · `handoff/projects/`로 내린다.
  - **「한 줄 상태」는 말 그대로 한 줄**이다 — 길어지면 그 내용은 프로젝트 파일에 있어야 할 것이다. 새 전역 실측도 `HANDOFF.md`가 아니라 `md/handoff-context.md`에 쓰고 목차에 한 줄만 올린다.
- **`projects/{name}.md`**: 「현재 상태」는 **그 시점의 상태로 덮어쓴다**(과거 세션 내용을 누적하지 않는다 — 누적분은 「세션 기록」으로). 세션 기록은 최신을 맨 위에 **최대 5개**, 넘치면 가장 오래된 것을 1~2줄로 압축 병합. **18KB 이내**.
- **`people/{person}.md`**: 본인 PC 정보만. **3KB 이내**.
- **⛔종결·주요 결정**: **전역이면 `md/decisions.md`에 전문 + `HANDOFF.md`에 결정문 한 줄**(2026-09-22 개정 — 종전 「HANDOFF에 최근 4건 전문」 폐기), 특정 대상에만 적용되면 프로젝트 파일. 애매하면 전역에 둔다 — 프로젝트 파일에 묻히면 다른 프로젝트 세션이 못 보고 재제안한다.
- **상한을 넘으면 지우지 말고 포인터로 압축한다** — 상세를 `md/` 정본으로 옮기고 원 위치에 **경고 한 줄 + 링크**만 남긴다. 삭제는 정보 손실이지만 포인터 압축은 주입량만 줄인다.
