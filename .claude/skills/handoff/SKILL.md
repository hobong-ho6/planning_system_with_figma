...
|---|---|---|---|
| **A** 기존 사용자 · 등록된 PC | `handoff.person` 있고 `people/{키}.md`에 이 hostname이 있다 | **없음** | 바로 브리핑하고 착수 |
| **B** 기존 사용자 · 새 PC | 사람키는 있는데 이 hostname이 파일에 없다 | **저장소 경로 1개** | `people/{키}.md` PC 표에 행 추가 · 세팅 열 ✅ |
| **C** 설정 누락 · 이력 추정 | 사람키가 없지만 **이 hostname이 어떤 `people/*.md`에 등록**돼 있다 | **「{추정된 사람} 님이신가요?」 1회** | 맞으면 그 파일의 identity로 `git config --global` 3값 안내 |
...
| **E** 사람키만 있음 | 사람키는 있는데 파일이 없다 | **이 PC 경로 · 담당 프로젝트** | `people/{키}.md` 생성 후 위와 동일 |
