...
2. **번역 후 — 검증 실행(필수, 자동+수동 모두)**: ① `scripts/validate_translation.py`(TranslationValidator)를 **실제 실행**(자동 3단계)하고, ② `md/check.md` 3단계 중 **자동 도구가 못 잡는 수동 항목**을 **신규·변경 키마다 직접** 검토한다 — 1단계(어색 표현·조사 누락·종결어미·마침표 일관성), 2단계(용어집 매칭이 진짜 위반인지 **오탐**인지 판별), 3단계(ja 한국어 직역·zh 번체 정자 `產`/`臺`·th 마침표 관습·구분자 표기). **자동 검증기 통과만으로는 게이트 미통과**다.
3. **검증 리포트 제시(필수 산출물 — 아래 셋 모두 대화에 남긴다)**: (a) `validate_translation.py`의 P0/P1/P2 요약, (b) **신규·변경 키별 수동 3단계 체크표**(1·2·3단계 각 결과), (c) 검증기가 보고한 **모든 P0/P1/P2 각 건의 처리 판정**(실제 위반→수정 / 오탐→사유 / 정책 보류→사용자 확인). (b)·(c)가 빠지면 — 자동 리포트만 있으면 — 그 번역은 **완료가 아니다**. (※ 과거 자동 검증기만 돌리고 수동 검토를 문서화하지 않아 누락된 사례가 있어 (b)·(c)를 강제한다.)
4. **통과 기준**: **P0 = 0건**이어야 출력·위키 반영·엑셀 생성으로 진행. P1/P2는 각 건 처리 판정을 남기고 사용자 확인.
