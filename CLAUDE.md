...
**세션이 시작되면 가장 먼저 프로젝트 루트의 `HANDOFF.md`를 읽고, "현재 상태"·"다음 할 일"을 확인한 뒤 한 줄로 브리핑한다.** 이는 SessionStart 훅(`.claude/settings.json`)으로 자동 주입되지만, 훅이 동작하지 않는 환경에서도 착수 전 반드시 직접 읽는다.

- **시점**: 사용자의 첫 요청에 응답하기 전, **토큰 요청보다도 먼저** 핸드오프를 확인한다. `HANDOFF.md`는 원격 조회가 아닌 **로컬 문서 읽기**이므로 토큰 우선 규칙의 "사전 탐색 금지"(Figma·Confluence 조회)에 해당하지 않는다.
- **순서**: ① `HANDOFF.md` 확인·한 줄 브리핑 → ② 토큰 요청(아래 토큰 우선 규칙) → ③ 실제 작업.
- **세션 종료 시**: 사용자가 "핸드오프 갱신"·"세션 정리"·"handoff"를 요청하면 `HANDOFF.md` 내부 주석의 작성 규칙에 따라 갱신한다.
