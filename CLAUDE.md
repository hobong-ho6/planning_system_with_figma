...
1. **트리거 (하나라도 해당하면 제안)**: ⓐ 규칙·절차(md/) 변경으로 사용자가 요청하는 방식이나 결과물이 달라질 때, ⓑ 새 기능·모드·메뉴가 추가됐을 때, ⓒ 용어집 버전이 올라갔을 때(가이드의 이력 표·임베드 JSON·전체 용어 표가 구버전이 됨), ⓓ 여러 커밋을 일괄 푸시해 누적 변경이 많을 때. 오타 수정·단건 산출물 커밋 같은 소소한 푸시는 제안하지 않는다.
