...
| 단계 | 화면(프레임) 범위 | 필터 적용 |
|:---:|------|------|
| 1단계 번역 (`md/translate.md`) | 이름이 `(New)`로 시작하는 프레임**만** 텍스트 추출·번역 | `(New)` 포함 필터는 **이 단계 전용** |
| 2단계 프로토타입 (`md/prototype.md`) | 페이지 직속 자식 프레임 **전체** + 모든 reactions 인터랙션 | **어떤 이름·크기 필터도 금지** — `(New)`/`Case` 무관 전부 포함 |
| 3단계 위키 (`md/wiki.md`) | 이름이 `Case`로 시작하는 프레임 **제외**, 나머지 전체 | `Case` 제외 필터는 **위키 Screen 표 전용** |
