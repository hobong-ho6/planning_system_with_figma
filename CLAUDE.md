...
2. **번역 후 — 검증 실행(필수)**: `md/translate.md` Step 5 절차로 `scripts/validate_translation.py`(TranslationValidator)를 **실제 실행**하고 `md/check.md` 3단계(한국어·용어집·언어별)를 수행.
3. **검증 리포트 제시(필수 산출물)**: P0/P1/P2 리포트를 **사용자에게 보여준다**. 대화에 검증 리포트가 없으면 그 번역은 **완료가 아니다**.
4. **통과 기준**: **P0 = 0건**이어야 출력·위키 반영·엑셀 생성으로 진행. P1/P2는 사용자 확인.
