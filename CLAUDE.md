...
> ⛔ **예외 — 용어집은 "제안"이 아니라 "필수 동반"이다 (2026-07-28 신설).** 용어집 버전이 올라가면 **승인을 묻지 않고 같은 작업에서 가이드 zip을 갱신해 전달**한다. 사용자가 **가이드 페이지의 용어집 탭에서 최신 전체 JSON을 복사해 Landpress CMS에 붙여넣는 것이 반영 창구**이고(Claude가 직접 PUT하는 경우에도 가이드는 사용자의 확인·이력 창구로 남는다), 가이드에 용어집 업데이트 이력이 실려 있기 때문이다. 갱신 대상 4곳(이력 표·전체 용어 표·수치 표기·임베드 JSON)과 검증 절차는 **`md/landpress.md` §5-1**. 게시(업로드)는 이 경우에도 **사용자가 수행**한다.
