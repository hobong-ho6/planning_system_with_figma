...
- **확인 시점 (셋 다)**: ① 세션 시작(핸드오프 규칙 ③ — 여기서는 기존대로 pull까지 수행) ② **공유 산출물을 쓰기 직전** — `guide/` 편집 시작 전, `dropweb/*.zip` 생성 직전, `HANDOFF.md`·`md/**` 갱신 직전, 커밋·푸시 직전 ③ 세션 시작 후 시간이 흐른 뒤(30분+) 다시 쓰기 작업으로 돌아올 때.
- **확인 방법**:
  ```bash
  git fetch -q && git rev-list --left-right --count HEAD...origin/main
  ```
  출력 `L R`에서 **R > 0이면 원격에 신규 커밋이 있다** → `git log --oneline HEAD..origin/main`으로 **무엇이 새로 왔는지 요약해 보고하고 사용자에게 pull을 권유**한다. 쓰기 작업은 pull 반영 전까지 진행하지 않는다.
- **가이드 버전 비교**: 가이드 작업이면 추가로 `guide/index.html` 내부 버전 ↔ 최신 `guide-v*` 태그(`git tag -l 'guide-*' --sort=-v:refname | head -1`)를 대조한다 — **태그가 라이브 정본 표식**이며, 태그가 로컬 소스보다 높으면 낡은 베이스다. 상세 절차는 `md/landpress.md` §5-1 0단계.
- **주의**: uncommitted 변경이 있으면 pull 전에 반드시 알린다. 그 변경이 **다른 세션의 것이면 커밋·stash 하지 않는다**(그 세션 소관 — 발견 사실만 보고). 리베이스 충돌이 나면 임의로 풀지 말고 충돌 파일 목록을 보고하고 사용자 확인을 받는다.
...
1-1. **⛔ 고유명사는 추측하지 않는다 — 공식 표기를 원본에서 확인한다(필수, 2026-09-13 신설)**: 브랜드명·제품명·기관명·법령명처럼 **세상에 정답이 있는 고유명사**는 번역 대상이 아니라 **조사 대상**이다. 기존 등록값(XLT·LPC)에 선례가 없으면 **웹에서 공식 출처로 확인한 뒤** 쓴다. **⛔ 한글 발음을 로마자로 옮겨 만들어내지 않는다.**
   - **확인 우선순위**: ① 기존 등록값(`fetch_xlt_registry.py`·LPC 조회) → ② **브랜드 공식 사이트·공식몰·기업 브랜드 페이지** → ③ 공식 SNS 계정명·도메인 → ④ 판매처(올리브영·다이소 등) 상품 페이지. **①에서 나오면 거기서 끝낸다**(등록값이 정본).
   - **대소문자·띄어쓰기까지 원문 그대로**. 소문자 브랜드(`hetras`·`innisfree`·`rom&nd`)를 임의로 대문자화하지 않는다.
   - **근거를 리포트에 남긴다** — 어느 출처에서 확인했는지 URL 또는 출처명을 (1a) 또는 (d-2)에 기록한다. 확인이 안 되면 **「확인 필요」로 표시해 사용자에게 올린다**(추정값을 확정처럼 쓰지 않는다).
   - 언어별 관례는 유지한다 — ja_JP는 가타카나 표기가 정본인 경우가 많고(`フィリミリ`), en·th·zh는 라틴 브랜드명을 그대로 쓰는 것이 기등록 관례다.
   - (계기: 2026-09-13 shopping_guide. 선례가 없다는 이유로 **필리밀리→`FEELIMEELY`·헤트라스→`Hetras`**로 추정해 5개 언어에 등록했는데, 공식 표기는 **`FILLIMILLI`**(CJ올리브영 브랜드 페이지)·**`hetras`**(공식몰·소문자)로 **둘 다 틀렸다**. 사용자 지적으로 웹 확인 후 재등록했다.)
1-2. **⛔ 신규 XLT 키는 「레지스트리 + 대기 키」 둘 다 대조한다(2026-09-21 신설)**: `fetch_xlt_registry.py --similar`(등록값)에 더해 **`scripts/check_pending_keys.py --wiki`**(다른 세션이 위키·게이트 리포트에만 올린 미등록 키)를 실행한다. 레지스트리만 보면 같은 날 다른 세션이 정의한 키를 못 봐 **같은 문구에 키가 두 벌 생긴다**(실측: `UF_voucher_pay_more_cu_*` ↔ `mini_voucher_cu_payment_done_benefit_*`). 히트하면 담당 FE 팀 판정까지 사용자에게 확인한다. 절차 정본 `md/translate.md` §2-1-c.
...
3. **검증 리포트 제시(필수 산출물 — 아래 모두 대화에 남긴다)**: (a) `validate_translation.py`의 P0/P1/P2 요약, (b) **수동 3단계 체크표**(전체 검증 시 발견 항목 전부 / 부분 작업 시 신규·변경 키), (c) 검증기가 보고한 **모든 P0/P1/P2 각 건의 처리 판정**(실제 위반→수정 / 오탐→사유 / 정책 보류→사용자 확인), (d) **추가 개선·제안(권장) 섹션**. (b)·(c)·(d)가 빠지면 — 자동 리포트만 있으면 — 그 번역은 **완료가 아니다**. (※ 과거 ⓐ 자동 검증기만 돌리고 수동 검토를 문서화하지 않았고, ⓑ 수동 검토를 신규 키로만 한정해 기존 키의 원문 오류(포이가츠·누리고다양한·perksand 등)를 놓친 사례가 있어 (b)·(c)와 전수 점검을 강제한다.)
   - **(d-1) 용어집 보완 권장(필수)**: 검증 중 ⓐ 용어집(API)에 없는 도메인 용어가 반복 사용되거나, ⓑ 자동 검증기가 **정상 의역을 P1로 반복 오탐**하면(= 용어집 매핑이 좁다는 신호), 용어집 보완이 더 근본적 해결이다. **해당 용어를 `md/glossary-backlog.md`(용어집 업데이트 대기 목록)에 등재**하고, 게이트 리포트 (d-1)에는 「**대기 목록 등재**」로 적는다. ⛔ **그 자리에서 반영 여부를 묻지 않는다** — 용어집 버전이 잘게 오르고 가이드 zip 동반 갱신이 매번 따라붙는 것을 막기 위해 **모아서 한 번에 리뷰·반영**한다(2026-09-14 사용자 결정). **⛔ 권장 표기 값은 추측하지 않는다** — `scripts/fetch_xlt_registry.py`로 **실사용 건수를 실측**해 정하고 리포트에 「실측 N건」을 남긴다. **실사용 0건 표기 권장 금지**(`md/check.md` 함정 2-1 「일반 규칙 ②」 · `md/landpress.md` §4 기준 4). **아래 반영 절차는 사용자가 「용어집 리뷰」를 요청한 시점에 수행한다**(발견 시점이 아니다). 용어집 반영은 **⛔ 사용자에게 방식을 먼저 묻는다** — ① Claude가 CMS API로 직접 반영 ② 사용자가 전체 JSON을 붙여넣기(`md/landpress.md` §10-5 1번). ①이면 **쓰기 전 CMS 로그인 상태를 확인하고, 미로그인(401)이면 사용자에게 로그인을 요청한 뒤 대기**한다(§10-1 — 자격증명은 Claude가 다루지 않는다). **묻지 않고 직접 쓰지 않고, 묻지 않고 산출물만 던지고 끝내지도 않는다.** 공개 조회 API는 읽기 전용이지만 **CMS 백오피스 API로는 저장·공개까지 가능하다**(§10). **전체 JSON 산출·전달 절차는 `md/landpress.md` 참조**(로컬 `glossary.json` 캐시 임의 수정 금지). **⛔ 사용자가 보완을 승인해 용어집 버전이 올라가면, 같은 작업에서 기획자 가이드 zip도 반드시 함께 갱신·전달한다**(`md/landpress.md` §5-1 — 가이드 페이지가 사용자의 용어집 반영 창구).
...
5. **리포트 완결성 강제 (완료 선언 직전 필수 실행 — 산출물 검사)**: 검증 리포트를 **`reports/gate/gate_report_*.md`로 저장**하고 **`python3 scripts/check_gate_report.py <리포트.md>` 실행 → exit 0(완결)** 을 확인한다. 3의 (a)~(d)·한국어 원문 교정·전수 점검 명시가 하나라도 빠지면 검사기가 **미완결로 차단**한다 — prose로만 있어 누락되던 것을 산출물 검사로 강제한다. exit 0 전에는 완료가 아니다.
...
| `md/glossary-backlog.md` | **용어집 업데이트 대기 목록** — 게이트 (d-1)에서 발견한 보완 필요를 **쌓아 두고 모아서 리뷰·반영**한다. 발견 시 등재 · 리뷰 요청 시 전건 제시 · 반영 후 삭제. **즉시 반영을 묻지 않는 것이 원칙** |
| `md/guide-backlog.md` | **기획자 가이드 업데이트 대기 목록** — 가이드는 **건건이 갱신하지 않고 모아서 한 번에** 반영한다(2026-09-15 결정). md/ 규칙·스크립트가 바뀌면 **묻지 말고 등재** · 사용자가 「가이드 업데이트」를 요청하면 전건 제시 후 반영 · 반영 후 삭제 |
...
| `md/handoff-context.md` | **전역 실측·사각지대 정본** — `HANDOFF.md`에서 이관(2026-09-22 포인터 압축). LPC 쓰기·스크립트 계약·검증기 사각지대·위키 편집·키 규칙 등 **15절**. `HANDOFF.md`에는 한 줄 경고 + 절 번호만 있으니 **그 작업 전에 해당 절을 연다** · 새 실측은 여기에 쓴다 |
| `md/decisions.md` | **전역 결정 사항 연혁 정본** — `HANDOFF.md`에서 이관한 전문(2026-09-15 포인터 압축). HANDOFF.md에는 최근 4건만 두고 나머지는 여기에 있다. **재작업·재제안이 의심될 때 먼저 확인** |
...
### 📣 기획자 가이드(dropweb) 갱신 규칙 — ⛔ 모아서 한 번에 (2026-09-15 사용자 결정)

**기획자 가이드는 건건이 갱신하지 않는다. 푸시할 때마다 「가이드를 업데이트할까요?」를 묻지 않고, `md/guide-backlog.md`에 등재만 하고 넘어간다.** 사용자가 **「가이드 업데이트」를 요청한 시점**에 대기 전건을 표로 제시하고 한 번에 반영한다.

> ⛔ **용어집은 예외가 아니라 같은 흐름이다.** 용어집 보완도 `md/glossary-backlog.md`로 모아서 반영하므로(2026-09-14 결정), **용어집 버전이 오르는 그 작업에서 가이드 zip을 함께 갱신**하면 된다 — 「필수 동반」(2026-07-28)은 그대로 유효하다. 용어집 반영이 없는 평상시 푸시에서는 가이드를 건드리지 않는다. 사용자가 **가이드 페이지의 용어집 탭에서 최신 전체 JSON을 복사해 Landpress CMS에 붙여넣는 것이 반영 창구**이고(Claude가 직접 PUT하는 경우에도 가이드는 사용자의 확인·이력 창구로 남는다), 가이드에 용어집 업데이트 이력이 실려 있기 때문이다. 갱신 대상 4곳(이력 표·전체 용어 표·수치 표기·임베드 JSON)과 검증 절차는 **`md/landpress.md` §5-1**. 게시(업로드)는 **Claude가 DropWeb MCP로 직접 수행**한다(2026-09-16 변경 — 아래 3ⓑ).
...
3. **사용자가 「가이드 업데이트」를 요청하면** — 대기 전건을 표로 제시해 반영 범위를 확정받은 뒤: ⓐ 가이드를 갱신한다 — **소스 정본은 git 추적 `guide/`다**(zip 직접 편집 금지, 베이스 확정·버전 번호·zip 생성·게시 태그는 `md/landpress.md` §5-1 0단계). 업데이트 이력 탭에 항목 추가(변경이 크면 버전 vN+1, 작으면 현행 버전 갱신). **⛔ 새 규칙 문서(`md/*.md`)·새 기능이 생겼으면 갱신 대상은 5곳 전부다(2026-09-03 신설 — GA.md를 v32에서 이력 카드·작업 모드·요청 예시에만 넣고 아래 ①②를 빠뜨려 v33으로 재발행한 실측)**: ① **가이드 탭 본문 전용 섹션**(규칙·표 구조·요청 방법 — 「담당 FE 팀 규칙」·「검증 게이트」와 같은 급) ② **시스템 가이드 탭 문서 지도 3곳**(흐름 카드 · 「하려는 일→읽을 문서」 표 · 파일 표) ③ 업데이트 이력(버전 요약 행 + 카드) ④ 상황별 작업 모드 카드 ⑤ 「이렇게 요청하세요」 예시 + 상단 네비 링크(데스크톱·모바일 2곳). 용어집 변경 시 용어집 탭의 이력 표·임베드 JSON·전체 용어 표를 최신으로 교체.
   - ⛔ **네비에 항목을 추가하면 반드시 렌더를 눈으로 확인한다 (2026-09-12 실측 — 같은 버전을 세 번 재발행함)**: ⓐ **항목마다 `<li>` 하나** — 기존 `<li>` 안에 `<a>`를 덧붙이면 두 메뉴가 한 항목처럼 **붙어서** 나온다(`GA EventOA 메시지`). ⓑ 네비는 `max-w-6xl` 안이라 **한글 항목이 늘면 폭이 넘쳐 전 항목이 2줄로 접힌다** — 항목 추가 시 `text-xs`·`gap` 축소·`whitespace-nowrap`으로 한 줄에 맞추고, 폭이 부족한 구간은 브레이크포인트를 올려(`md:`→`lg:`) 햄버거로 넘긴다(`navToggle`·`navLinks`·`navLinksMobile` **세 곳 동시 변경**). ⓒ 구조 검사만으로는 안 보이므로 **headless Chrome으로 1920·1440px 헤더를 캡처해 한 줄인지 확인**한 뒤 zip을 만든다(`--user-data-dir` 지정·`--virtual-time-budget=6000` 없으면 CDN 로딩으로 멈춘다). ⓑ `guide/` 커밋 + 생성한 zip을 **Claude가 직접 게시**한다(2026-09-16 사용자 결정 — 종전 「게시는 사용자가 수행」을 대체). ⛔ **재배포는 사용자 승인을 받은 뒤**에만 한다. 업로드는 **REST `PUT /api/sites/{siteId}`**(MCP 토큰 인증 · `file`+`name` multipart)이고 **MCP `update_site`는 쓸 수 없다** — `file_path`가 DropWeb 서버의 파일시스템 경로라 로컬 zip을 못 읽는다(2026-09-16 실측). 게시 후 **MCP `get_site`로 수정일·원본 파일명**과 **라이브 `<title>` 버전**을 대조해 확인한다. 절차·대상 사이트는 `md/dropweb-guide.md` §8. 확인 후 `git tag guide-vN`.
...
| `check_pending_keys.py` | **미등록 대기 키 대조** — 게이트 리포트 + **위키 전문 검색(CQL)** 으로 「다른 세션이 오늘 위키에만 올린 키」를 찾는다(읽기 전용) | 1단계 Step 2-1-c — **신규 키 확정 직전 필수** |
