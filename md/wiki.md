...
- **⛔ `xlt`가 스레드에 얽힌 경우의 정책 포함/제외 (핵심 — 누락 재발 방지)**: 본문을 trim·소문자화했을 때 **딱 `xlt`인 코멘트/답글은 "이 문구를 번역하라"는 번역 마커일 뿐, 정책 텍스트가 아니다.** 두 갈래로 나눠 처리한다.
  - **(A) 루트 자체가 순수 `xlt`** → Description **정책 텍스트에 넣지 않는다**(번호도 매기지 않는다). 그 위치는 **XLT 표의 `No`·이미지 어노테이션(ⓝ) 번호로만** 쓴다(코멘트 선별/단일 프레임 모드의 XLT No 체계, `md/translate.md`). 즉 `xlt` 마커의 "본문"을 `1. xlt`처럼 Description에 쓰지 않는다.
  - **(B) 루트가 실제 정책인데 `xlt` 답글이 달림** → 그 **루트 정책은 반드시 Description에 포함**한다(번호 부여). `xlt` 답글은 "이 루트 문구도 번역 대상"이라는 마커이므로 **`↳`로 표시하지 않는다**(정책 내용이 아님). ⚠️ 이 케이스에서 **루트 정책을 통째로 누락**하거나(2026-07 36558 root 정책 2건 누락) **정책 답글을 누락**한 사례가 있어 강제한다 — `msg.lower()=='xlt'`인 **루트만** 건너뛰고, 그 외 루트와 `xlt`가 아닌 모든 답글은 전부 반영한다.
  - **정리**: `build_threads` 결과에서 **순수 `xlt` 루트만 Description에서 제외**, 나머지 루트는 전부 정책으로 포함하고, 각 루트의 답글 중 **순수 `xlt`가 아닌 답글은 모두 `↳`로 포함**한다. **코멘트 선별 모드로 XLT만 번역한 경우에도**(`md/translate.md` 코멘트 선별 모드) XLT 컬럼·다국어 섹션만 채우지 말고 **Description을 이 규칙대로 함께 갱신**한다.
