...
> 🔑 **토큰은 필요한 단계에서 — 규칙 정본은 CLAUDE.md '⛔ 토큰 규칙'(2026-09-18 개정), 확보·검증 절차는 아래 Step 0.** 조회·범위 파악·초안 작성은 토큰 없이 진행하고, Figma REST·Confluence 쓰기 직전에 키체인 → 없으면 요청 순으로 확보한다.
...
| Figma URL | ✅ | **페이지** URL → 전체 화면 / **프레임** URL → 그 프레임만 (모드 분기). URL 대신 **페이지·섹션 + 프레임 이름**으로 지정해도 된다 → "대상 프레임 지정 방식" 참조 | `.../Web3?node-id=55762-1104` |
...
### 대상 프레임 지정 방식 — URL이 없어도 이름으로 특정한다 (2026-08-05 신설)

사용자가 프레임 URL(`node-id`)을 주지 않고 **"시즌3 페이지의 `(OA)Reward Confirm` 프레임"** 처럼 **위치 + 프레임 이름**으로만 지정하는 경우가 많다. 이때 URL을 다시 요구하지 말고 아래 절차로 특정한다 (2026-08-05 실측: OA 프레임을 이 방식으로 특정).

1. **범위 확정** — 파일 키 + 페이지(CANVAS) node-id를 확보한다. 사용자가 "시즌3 페이지"처럼 페이지 이름만 말하면 `GET /v1/files/{fileKey}?depth=1`로 페이지 목록을 조회해 이름으로 찾는다.
2. **직속 자식 재조회** — `GET /v1/files/{fileKey}/nodes?ids={pageId}&depth=1`로 **그 페이지의 직속 자식 프레임 목록(이름·id)을 매번 새로 조회**한다. 이전 run의 `frames.json`·노트에 적힌 node-id를 그대로 쓰지 않는다 (**캐시 금지 규칙** — 프레임은 이름이 바뀌거나 삭제·재생성되어 id가 달라진다).
3. **이름 필터** — 사용자가 말한 이름으로 **완전일치 → 부분일치** 순으로 후보를 좁힌다. 섹션(예: `OA`, `Promotion Page`)을 함께 말했으면 그 섹션(부모 FRAME/SECTION) 하위로 먼저 한정한다.
4. **⛔ 동명 프레임 확인 (필수)** — 한 파일 안에 **같은 이름의 프레임이 여러 개** 있을 수 있다. 실측: `(OA)Reward Confirm` 2개 · `(Unifi mini) - Logged in` 2개.
   - **후보가 2개 이상이면 임의로 고르지 않고 사용자에게 확인**한다. 확인 시 각 후보의 **node-id + 부모 섹션 이름 + 프레임 크기**를 함께 제시해 구분할 수 있게 한다.
   - 후보가 0개면 조회한 직속 자식 이름 목록을 그대로 보여주고 사용자에게 어느 것인지 묻는다 (**이름이 바뀌었을 가능성**을 먼저 의심한다).
5. **확정 후 보고** — 처리에 들어가기 전 `{프레임 이름} = {node-id}`를 한 줄로 알린다. 이후 절차는 Mode B와 동일하다.

---

## 위키 생성 모드 (신규 페이지 생성 — 기존 페이지 업데이트가 아닌 경우)
...
1. **참조 템플릿 (형태 기준)**: **`templates/wiki_template.xml`** (raw storage XHTML 스냅샷)의 구조를 그대로 사용해 생성한다.
   ...
   - **형제 문서 구조 준용 (템플릿보다 우선 — 단, 형제 1건만 보고 일반화하지 않는다)**: 생성 위치의 **같은 부모 아래 형제 페이지들이 실제로 일관된 변형 구조**를 쓰고 있으면 그 구조를 따른다 — 같은 폴더 안에서 문서 형태가 갈리지 않는 것이 우선이다. **형제가 여러 건이면 최소 2건 이상을 대조**하고, 서로 다르면(정본 템플릿을 따르는 것과 벗어난 것이 섞여 있으면) **정본(`templates/wiki_template.xml`)을 따르는 쪽을 기준으로 삼는다** — 형제 1건의 구조만 보고 "이 폴더의 규칙"으로 일반화하지 않는다.
     - ⚠️ **실측 오류 정정(2026-08-28)**: 이전 기재는 "`[Hogeun]` 하위는 `Background` 없이 `Schedule`을 쓴다"(2026-07-30 실측)였으나, 이는 형제 페이지 **`K-Pick 상품 환산가 및 환율 정보 표기`(pageId 4620240898) 1건만 확인**하고 일반화한 것이었다. 같은 부모(`[Hogeun]`, pageId 3910828993) 아래 **`IAB로 Unifi 접근 시, LIFF로 이동`(pageId 3751027285)은 정본 템플릿 그대로**(`Background` 포함·`Schedule` 없음·사이드바 레이아웃·History 확인 칸 체크박스)다. 즉 `4620240898` 쪽이 정본에서 벗어난 예외였다 — **`[Hogeun]` 하위의 기본값은 정본 템플릿**이며, `4620240898`은 교정 후보로만 남긴다(소급 수정은 별도 확인 후).
...
5. **토큰 규칙 적용**: 생성(쓰기) 직전에 Confluence PAT를 키체인 → 없으면 요청 순으로 확보·검증한다. Figma 화면 반영이 동반되면 그 단계에서 Figma PAT도 같은 순서로 (CLAUDE.md '⛔ 토큰 규칙').
6. **⛔ 동명 페이지 선확인 (생성 전 필수 — 중복 생성 방지)**: 생성 직전 **부모 페이지의 자식 목록**(`GET /rest/api/content/{parentId}/child/page?limit=50`)에서 **같은 제목이 이미 있는지 확인**한다. ⚠️ **CQL 검색(`title ~`·`siteSearch ~`)으로 존재 여부를 판정하지 않는다 — 방금 만든 페이지는 인덱싱 전이라 잡히지 않는다**(2026-09-15 실측: 생성 12:40인 페이지가 같은 날 오후 CQL에서 0건으로 나와 「위키 미반영」으로 오판했다). 존재 확인은 항상 `child/page` 목록으로 한다. Confluence는 **같은 space에 동일 제목 페이지를 만들 수 없고**, 제목을 미묘하게 바꿔 만들면 중복 문서가 생겨 혼란해진다.
...
- **⛔ Jira 티켓을 받으면 티켓을 조회해 부수 링크까지 확장한다(권장 — 실측 유용)**: summary·status·description **그리고 코멘트·첨부까지** 확인하고, 거기 들어 있는 **관련 링크(PIA Review 티켓·Slack 스레드·관련 이슈·Figma)를 Related Docs 행으로 추가**한다. 사용자가 티켓만 줘도 문서의 참조 맥락이 한 번에 갖춰진다. 티켓 제목·상태가 위키 제목과 어긋나면(오타 등) **지적해 보고**한다.
  ...
  - **조회 수단**: `jira_get_issue`(MCP)가 있으면 그걸 쓰되 **MCP는 세션에 따라 끊긴다** — 그때는 REST로 폴백한다(`GET /rest/api/2/issue/{KEY}` · `GET /rest/api/2/issue/{KEY}/comment`). ⛔ **Jira는 Confluence PAT로 401** — 별도 **Jira PAT**를 키체인(`jira-pat`) → 없으면 그 단계에서 요청한다(토큰 규칙).
...
### Policy 섹션 작성 포맷 (사용자 확정 방식, 2026-08-04 — 생성·업데이트 공통)

**`Specification > Policy`는 산문·소제목이 아니라 `구분 | 설명` 2열 표 하나로 쓴다.** 사용자가 직접 작성한 럭키볼 친구초대 캠페인(pageId=4479306980) Policy가 정본 포맷이며, 신규 생성·기존 문서 업데이트 모두 이 형태로 맞춘다.

```xml
<h2>Policy</h2>
<table class="wrapped"><colgroup><col /><col /></colgroup><tbody>
<tr><th scope="col">구분</th><th scope="col">설명</th></tr>
<tr><td>{기능·화면·단계 이름}</td><td><ul style="list-style-type: square;">
  <li>{정책 문장}</li>
  <li>{정책 문장}</li>
</ul></td></tr>
</tbody></table>
...
| 항목 | 규칙 |
|---|---|
| 구조 | `<h2>Policy</h2>` 바로 아래 **표 1개**. `<h3>` 소제목·머리말 문단·섹션 번호(`1.`·`2.`) 금지 |
| 헤더 | `구분` \| `설명` 2열 고정(`<th scope="col">`), `<colgroup><col /><col /></colgroup>` 포함 |
| 구분(좌) | **기능·화면·단계 이름**을 명사구로. 예: `프로모션 배너 노출` · `친구 초대 링크 생성` · `피초대자 프로모션 페이지` · `초대자의 럭키볼 뽑기` · `프로모션 종료` |
| 설명(우) | `<ul style="list-style-type: square;">` + `<li>`. 한 줄이어도 표 셀에 문장을 그냥 두지 말고 불릿으로 쓴다 |
| 문체 | **완결된 평서체 서술문**(`~한다`·`~된다`·`~노출된다`). 조건은 `~인 경우`·`~ 시`로 앞에 두고 결과를 뒤에 쓴다 |
| 행 순서 | **사용자 여정·화면 흐름 순** (노출 → 진입 → 조건 판정 → 수행 → 완료 → 종료). 마지막에 `프로모션 종료`처럼 종료 시 동작을 둔다 |
| 금지 | 출처 티켓 링크·`(TBD)`·굵기 강조·`<code>`·이모지를 정책 문장에 섞지 않는다 — 표는 **정책 서술 전용**이다 |

**출처·미확정 처리**: 티켓·Slack에서 끌어온 근거는 Policy 표에 쓰지 않고 **Related Docs**(티켓·Discussion 행)에 남긴다. 아직 확정되지 않은 항목은 **표에 쓰지 않는다** — 확정된 정책만 행으로 올리고, 미확정 사항은 사용자에게 보고해 결정 후 추가한다(정보 없는 섹션 처리 규칙과 동일).
...
### Step 0: 토큰 확보 (필요한 단계 직전 — 2026-09-18 개정)

**Step 1(대상 페이지 확인·MCP 조회)·범위 파악·본문 초안은 토큰 없이 진행한다.** Figma REST 수집 또는 Confluence 쓰기에 닿기 직전에 아래를 수행한다.

0. **키체인 먼저**: `security find-generic-password -s figma-pat -w` · `-s confluence-pat -w`. 있으면 값을 출력하지 않고 환경변수로 넘기고 2의 검증만 한다. 없는 토큰만 1로 요청한다.
1. 사용자에게 **없는 토큰만** 요청한다 (어느 단계에 쓰는지 함께 적는다):
...
> ✅ 토큰 없이 가능한 작업(범위 파악·페이지 조회·구조 파싱·초안)은 먼저 진행한다. 끝내 토큰이 없으면 쓰기 단계만 「사용자 차례」로 남긴다 (CLAUDE.md '⛔ 토큰 규칙').
...
- **어휘 정본 = `md/IA.md`**: 주기능·부기능·세부기능 단어는 IA.md의 트리에서 고른다. 트리에 없는 새 기능이면 **IA.md에 먼저 추가**(사용자 확인)한 뒤 부여한다. IA가 바뀌면 IA.md만 갱신한다(다른 md에 IA 중복 기재 금지).
- **⛔ 검토 게이트(필수 — 예외 없음)**: Screen ID 부여 전 `{Figma 프레임명 → Screen ID}` **매핑 표를 사용자에게 제시하고 검토·승인받은 뒤에만** 위키에 반영한다. 임의 확정 금지.
- **소급 금지**: 이 규칙 이전에 프레임명 기반 Screen ID로 작성된 기존 위키(예: 럭키볼 친구초대 캠페인 pageId=4479306980)는 **그대로 유지**한다 — Mode B 행 매칭도 그 페이지에서는 기존 방식(프레임명 셀 매칭)을 계속 쓴다. 새 규칙은 신규 부여분부터 적용.
- ❌ `SC-01` 같은 무의미 임의 번호 금지 (구조화 ID가 아님)
- ❌ node-id를 Screen ID 셀에 포함하지 않는다 (예: `(51762:2592)` 같은 괄호 병기 금지)
...
- **답글(스레드) 반영 필수 — 모든 답글을 빠짐없이**: 한 핀(코멘트)에 답글이 여러 개 달린 스레드는 **루트 코멘트 번호 아래 줄에 답글 본문만** `created_at` 시간순으로 이어 표시한다(작성자·날짜 없이 본문만, 들여쓰기 마커 `↳` 사용). 번호(`1.`, `2.`, …)는 **루트 코멘트에만** 부여하고 답글에는 부여하지 않는다 — 핀은 루트에만 존재하기 때문이다
  - **⛔ 2번째 이후 답글 누락 금지 (실측 재발)**: 답글이 2개 이상이면 **모든 답글**을 순서대로 넣는다. 첫 답글만 반영하고 이후 답글을 빠뜨린 사례가 있었다(2026-07 `Unifi mini - 피초대자 OA 팔로우`(37141)에서 `선택하면 럭키볼을 뽑는다.`, `피초대자 OA 팔로우 X`(37584)에서 `선택 시, OA 페이지가 바텀시트로 노출된다.` 누락). **반드시 `fetch_threads`의 `log_self_check`가 출력하는 루트별 `attached replies: N`(= `xlt` 답글 제외한 실제 정책 답글 수)과 위키 Description의 `↳` 개수를 대조**해 전부 반영됐는지 확인한다.
- **미해결(resolved_at이 null) 코멘트만 포함**한다 — 해결된 코멘트는 이미 반영된 정책이다 (루트가 해결됨이면 그 스레드의 답글도 함께 제외)
- **⛔ `xlt` 마커도 정책과 함께 "통합 번호" 부여 (번호 체계 일치)**: 미해결 루트 코멘트는 **정책이든 순수 `xlt` 마커든 모두 y좌표 순으로 하나의 통합 번호(`1.`, `2.`, …)** 를 부여한다. **이미지 어노테이션 ⓝ·XLT 표의 `No`·Description 번호는 모두 이 통합 번호 하나로 맞물린다**(같은 번호 체계).
  - **Description 본문에는 정책 루트만 쓴다 (2026-08-07 관행 정본화)**: 정책 루트는 번호 + 본문(+`↳` 답글)을 Description에 쓰고, **순수 `xlt` 마커 루트는 Description에 쓰지 않는다** — 그 번호는 **XLT 표의 `No`로 나타난다**. 즉 Description 번호와 XLT `No`는 **상보적으로 나뉜다**(정책 루트에 `xlt` 답글이 달려 그 정책 문구 자체가 번역 대상인 경우만 양쪽에 같은 번호가 등장).
    - 실측(시즌3 12프레임 전수, 2026-08-07): `(Promotion) Draw Done` Desc `1` ↔ XLT `2·3` · `(Popup)JP Only` Desc `1` ↔ XLT `2·3·4` · `(Promotion) Button Case` Desc `1·3·4·6` ↔ XLT `2·3·5·6`(3·6은 정책에 `xlt` 답글) · `(Promotion) Promotion page` Desc `5·6·11·14·17·29` ↔ XLT 그 외 23개. **12프레임 전부 번호 체계가 정합**했다.
    - ⚠️ **이전 규칙은 순수 `xlt`도 Description에 `2. xlt`로 쓰라고 했으나** 시즌3 16프레임 전부가 생략했고, 번호 정합은 유지됐다. `xlt`라는 글자는 기획자·FE에게 의미가 없고 같은 정보가 XLT 표에 이미 있어 **관행을 정본으로 채택**했다(대안: 30여 행 일괄 추가 — 기각).
  - **XLT 표의 `No` = 그 XLT 문구를 가리키는 코멘트의 통합 번호**다. 순수 `xlt` 마커가 가리키는 문구는 그 마커의 번호를, **정책 루트에 `xlt` 답글이 달려 그 정책 문구 자체가 XLT 대상이면 그 정책의 통합 번호가 곧 XLT No**가 된다. 따라서 정책이 앞 번호를 차지하면 XLT No는 `1`이 아니라 그 뒤 번호부터 시작할 수 있다(예: 37141은 정책 `1` 뒤 XLT가 `2`·`3`).
  - **`↳` 답글 규칙**: 정책 루트의 답글 중 **순수 `xlt`가 아닌 답글만** `↳`로 이어 표시한다(위 "모든 답글" 규칙). `xlt` 답글은 "이 루트 문구도 번역 대상"이라는 마커일 뿐이라 `↳`로 쓰지 않고, 대신 그 정책의 통합 번호를 XLT No로 쓴다.
  - **코멘트 선별 모드로 XLT만 번역한 경우에도** XLT 컬럼·다국어 섹션만 채우지 말고 **Description·이미지 어노테이션을 이 통합 번호 규칙대로 함께 갱신**한다.
  - ⚠️ (2026-07 실측) 이 통합 번호를 **XLT만 `1`부터 따로 매기고 정책·`xlt`를 분리**해 번호가 어긋난 사례(37141·36558·37584)가 있어 **원 규칙(통합 번호)으로 강제**한다. 반대로 정책 루트·정책 답글을 통째로 누락한 사례(36558 root 정책, 37141·37584 답글)도 있었으니 **순수 `xlt`가 아닌 루트·답글은 절대 빠뜨리지 않는다**.
...
>
> **⛔ 좌표 정규화 (필수 — 통합 번호의 전제):** 코멘트 핀의 `node_offset`은 **앵커 노드 기준 상대좌표**다. 핀은 프레임이 아니라 **하위 프레임/컴포넌트에 앵커될 수 있고**, 그 경우 y 원값을 그대로 정렬하면 통합 번호가 화면 순서와 어긋난다(2026-07-27 실측: 63411-25034에서 하위 프레임 앵커 핀 7개가 y=83~339로 보고돼 화면 하단(y=1458~1714) 유의사항인데 상단 번호로 끼어들었고, 직전 세션이 y=1423 핀을 "잔존 핀"으로 오판). **번호를 매기기 전 반드시 `collect_node_boxes(frame_doc)` 결과를 `fetch_threads(..., node_boxes=, frame_origin=)`으로 넘겨 프레임-상대좌표로 정규화**한다. `log_self_check`는 다중 앵커+미정규화면 ⚠️ 경고를 출력한다 — 이 경고가 보이면 번호 매기기 전에 정규화부터 한다.
>
...
##### ⛔ `collect_frames.py` 실측 함정 4종 (HANDOFF.md에서 이관 · 전부 실제 오판 사례)

이 스크립트의 자동 산출을 **그대로 믿으면 안 되는 지점**이다. 매번 아래를 확인한다.

| # | 함정 | 증상 / 실측 | 대응 |
|---|---|---|---|
| ⓐ | **`xlt` 마커 판정이 규칙보다 좁다** | 스크립트는 「본문이 정확히 `xlt`」만 잡는데 CLAUDE.md 규칙은 「`XLT`로 시작」이다. 설명 뒤 끝줄 `xlt`(2026-09-03 브릿지 mini: 10건 중 **4건 누락**), 구두점 `xlt,`(2026-09-08), **결합 마커** `xlt\n다른텍스트`(클리닉 38건 중 **7건**)를 놓친다 | **마커 집합은 규칙대로 직접 산정**한다 — `threads` 원본을 **줄 단위 + 첫/끝 토큰, 구두점(`,.:;`) 제거** 기준으로 순회. 핀 색도 다시 렌더. ⚠️ 스크립트는 **아직 좁은 판정 그대로**(반영 대기) |
| ⓑ | **코멘트→텍스트 좌표 매칭이 더미를 잡는다** | 박스가 겹치면 엉뚱한 노드가 선택된다 — `cypress`(네트워크값)·`KLAY`(코인심볼)가 뽑혀 **3건 오매칭**(2026-08-24 kaiawallet). **어노테이션 이미지 육안 확인으로만** 드러났다 | 매칭 결과를 **번역 전에 이미지와 대조**한다 |
| ⓒ | **프레임 이름·정책 번호가 세션 중에 바뀐다** | `상품권`→`카테고리 1`로 이름이 바뀌고, 코멘트가 앞에 삽입되면 **정책 번호가 전부 밀린다**(2026-09-08 unifi-mini-v2: 카테고리 정렬 6→7건) | Screen ID·표기용 이름은 **이전 run의 `frames.json`이 아니라 Figma에서 새로 조회**. 행·크롭 재생성은 번호가 아니라 **(본문 문구, 답글, 원 y좌표) 3중 키**로 다시 찾는다 — 문구가 같은 정책과 `xlt` 한 단어 정책이 **첫 일치 오매칭**을 낸다 |
| ⓓ | **팝업 프레임의 배경은 구버전 화면일 수 있다** | 삭제된 문구가 노드로 살아 있어 「화면에 있으니 유지」로 오판(2026-08-24) | **키 제거·유지 판정의 정본은 전체 페이지 프레임**이다. 노드 존재만으로 판정하지 않는다 |

- **집계 표(Admin/API/LPC) 크롭은 정책 위치 밴드(y±110pt, 전체 폭)로 충분하다** — Figma에 이미 구분 박스가 그려진 경우가 많아 정교한 박스 검출 없이도 맞아떨어진다(2026-09-10 실측).
...
**⛔ 번호·좌표 기준 (깔끔한 배치 규칙 — 매 어노테이션마다 항상 수행, 예외 없음):**
- **⓪ 좌표 정규화 선행 (필수)**: 핀 좌표는 Step 3의 좌표 정규화(`collect_node_boxes` → `fetch_threads(node_boxes=, frame_origin=)`)를 거친 **프레임-상대좌표만** 쓴다. 원값(`node_offset`)으로 번호를 매기면 하위 노드 앵커 핀이 엉뚱한 순번을 차지한다(2026-07-27 25034 실측).
- **글자 가림 금지**: 텍스트-매칭 원의 중심은 텍스트 왼쪽 끝에서 **10pt 왼쪽**(`rel_x = text.x - 10`, 경계 clamp)에 둔다 — 원이 첫 글자를 덮으면 안 된다(과거 v5 이미지에서 ⓝ가 첫 글자를 가려 지저분했던 실측 개선).
- **겹침 0 검증 (산출물)**: 렌더 전 전 쌍의 중심거리를 검사해 `지름+1pt` 미만이면 뒤 번호 원을 **같은 행에서 우측/여백으로 최소 이동**해 해소하고, `overlaps: []` 확인 결과를 대화에 남긴다. 정책 핀도 겹치면 이동 가능(핀 위치는 근사값 — 단, 가리키는 대상이 명확히 남는 위치로만).
- **색 구분 고정**: **정책(순수 정책 + xlt 답글 달린 정책) = 빨강, 순수 `xlt` 마커 = 파랑.** 위키 캡션 문구("빨강=정책·파랑=XLT 마커")와 실제 렌더 색이 **반드시 일치**해야 한다(과거 캡션은 색 구분인데 전부 빨강으로 렌더된 실측 재발 방지).
- **렌더 후 육안 검증 (모든 어노테이션 필수)**: 팝업 겹침 화면만이 아니라 **모든 어노테이션 이미지를 렌더 직후 열어** ⓝ가 의도한 텍스트 옆에 있는지, 번호가 위→아래로 증가하는지, 디자인이 최신 상태(구버전 렌더 아님)인지 확인한다.
- **번호 3면 일치 확인**: 이미지 ⓝ = Description 통합 번호 = XLT 표 No가 1:1인지 최종 대조 후 첨부한다.
- **정책 코멘트 모드(기본) — 통합 번호**: `no`는 **Description 통합 번호**(정책+`xlt` 마커를 y 오름차순으로 함께 매긴 번호 — 위 'xlt 마커도 통합 번호' 규칙). 이미지 ⓝ = Description 통합 번호 = XLT 표 No, 1:1. **좌표(ⓝ 위치)는 가독성을 위해 항목 종류로 나눠 찍는다**:
  - **XLT 문구가 매칭된 항목**(순수 `xlt` 마커 + `xlt` 답글이 달린 정책): **매칭 텍스트 bbox의 왼쪽 끝 x·세로 중앙 y**(`rel_x = text.x - frame.x`, `rel_y = text.y - frame.y + text.h/2`)에 찍는다 — 문구가 조밀한 화면(수십 개)에서도 각 텍스트 줄 좌측에 번호가 정렬돼 깔끔하다. 핀 위치가 아니라 **매칭 텍스트 위치**임에 주의(핀은 텍스트와 어긋날 수 있다). ⚠️ 팝업/모달 겹침 시 배경 텍스트로 오정렬될 수 있으니 렌더 후 육안 확인(키↔텍스트 매칭 시 코멘트 핀 최근접으로 중복 문구를 구분).
  - **매칭 텍스트가 없는 순수 정책 항목**(번역 대상 문구가 없는 안내·네비게이션 정책): **코멘트 핀 위치(frame-relative)**에 찍는다.
  - (실측 근거: 초대자 8개 프레임에서 전원 핀 위치로 찍으니 번호가 흩어져 지저분했고, 매칭 텍스트 좌측 정렬로 바꿔 정돈함. 2026-07.)
...
- **⛔ 팝업/모달 겹침 시 육안 검증(필수)**: 당첨/알림 팝업·모달이 배경 화면 위에 겹친 프레임은 코멘트 핀 좌표가 **팝업 뒤 배경 텍스트**로 오매칭될 수 있다(좌표 매칭이 z-order 미인식 — 배경의 작은 bbox를 잡음). 어노테이션 이미지를 렌더해 **핀이 팝업의 의도한 문구 위에 있는지 육안 확인**하고, 어긋나면 팝업 실제 문구로 수동 교정 후 첨부한다(2026-07 `mini_luckyball_voucher_musinsa` 오매칭 실측 — 팝업 `100JPYC 당첨됐어요!` 위 핀이 배경 `무신사 바우처`로 잡힘). `md/translate.md` 코멘트 매칭 '엣지 케이스'와 동일 규칙.
...
- **이 단계는 위키 토큰(Confluence PAT)이 있어야 가능하다** — 토큰 규칙대로 이 단계 직전에 키체인 → 요청 순으로 확보하고, 끝내 없을 때만 아래 fallback으로 우회한다
...
## 마스터 페이지 갱신 (Release History ↔ Summary 정합)

> 마스터 페이지(`[Master] Lucky Ball Promotion`·`Mission and Reward`·`Unifi mini`·`Wallet Mode`)는 **하위 문서의 버전별 정책 요약**을 모은다. 개별 화면 문서와 구조가 달라 별도 절차를 둔다. (2026-08-07 신설 — 세션 #13에서 두 번 유용했던 방법을 규칙화)

### 1. 정합 판정 (가장 먼저)

**`Release History` 데이터 행 수 = Summary `<h3>버전 N` 블록 수** 여야 한다.

```python
# 앵커는 반드시 <h1>Release History</h1> — History 행 본문에 "Release History"라는
# 글자가 들어가 오집계된 실측이 있다
i = s.index('<h1>Release History</h1>')
rows = s.count('<td class="numberingColumn">', i, s.index('</table>', i))   # 데이터 행
blocks = len(re.findall(r'<h3>버전 \d+', s))
```

- 릴리즈 버전이 미확정이면 Summary 제목은 `[PL] Unifi (버전 미정)`으로 둔다.
- **행이 삽입되면 Summary 블록 번호를 밀어 1:1을 복구**한다 — 사용자가 Release History에 행을 넣는 것이 정상 흐름이고 Summary가 뒤늦게 따라간다(실측 2회: Lucky Ball 8행↔7블록).

### 2. 이미 정합인데 "업데이트" 요청을 받으면 — 직전 버전과 diff

정합 상태라 반영 대상이 안 보일 때, **추측하거나 되묻지 말고** 라이브와 직전 버전을 비교해 **사용자의 마지막 편집을 특정**한다.

```bash
# 직전 버전 storage
curl -s -H "Authorization: Bearer $CONFLUENCE_PAT" \
  "$BASE/rest/api/content/{pageId}?status=historical&version={N-1}&expand=body.storage"
```

- 태그 경계(`(?<=>)(?=<)`)로 쪼개 `difflib`로 비교한다 — storage는 한 줄이라 라인 diff가 무의미하다.
- 실측: `[Master] Unifi mini`가 3↔3 정합이라 대상이 안 보였는데, v6과 diff해 **사용자가 방금 추가한 `K-Pick 노출 정책` 링크**를 특정하고 그 편집이 요구하는 Summary 변경을 반영했다.

### 3. 구·신 정책이 갈리면 ①As-is → ②To-be 이력으로

참조 문서가 구·신으로 나뉘면 **최신 것만 남기지 말고** 같은 블록 안에 이력으로 배치하고 **어느 쪽이 최신인지 명시**한다.

- 최신만 남기면 왜 바뀌었는지가 사라지고, 구 서술을 그대로 두면 한 블록에서 상반돼 보인다.
- 실측: mini 마스터 버전 2에서 K-Pick 노출 조건을 **①**(JP IP만) → **②**(JP IP **또는** KR IP + 언어 non-KR로 확장)으로 배치하고 ②가 ①을 대체했음을 명시. 블록 말미에 떨어져 있던 KR IP 문장을 ①로 옮겨 상반을 해소했다.

### 4. 주의

- **전역 금지 가드는 이력 기록과 충돌한다** — `"(버전 미정)" not in body` 같은 검사가 **History에 인용된 `<code>(버전 미정)</code>`** 을 잡은 실측이 있다. 가드는 **`<h3>` 제목 대상**으로 한정한다.
- `check_wiki_storage.py`가 마스터 페이지의 기존 3컬럼 표를 4컬럼 위반으로 잡을 수 있다. **편집 전부터 있던 위반**이면 `pre`로 찍어 **「편집 전/후 위반 목록 동일 = 신규 위반 0건」** 을 통과 근거로 삼는다(소급 변경 금지 규칙 우선).
