...
- **⛔ `xlt` 마커도 정책과 함께 "통합 번호" 부여 (원 규칙 — 번호 체계 일치)**: 미해결 루트 코멘트는 **정책이든 순수 `xlt` 마커든 모두 y좌표 순으로 하나의 통합 번호(`1.`, `2.`, …)** 를 부여하고 **Description에 함께 포함**한다. 정책 루트는 본문(+`↳` 답글)을, 순수 `xlt` 마커 루트는 본문 그대로 **`xlt`** 를 그 번호로 표기한다(예: `2. xlt`). **이미지 어노테이션 ⓝ·XLT 표의 `No`·Description 번호는 모두 이 통합 번호 하나로 맞물린다**(같은 번호 체계).
