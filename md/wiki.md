...
- 날짜는 업데이트 당일(`YYYY-MM-DD`), 버전은 직전 버전에서 증가, 작성자는 `작성자 (Claude 자동 생성)`
...
5. **표에 surgical 삽입/치환**
   ...
   - **History 표에 변경 행 1줄 추가 (필수)**: `날짜 | 버전 | "{프레임명} 행 추가/갱신" | 작성자` — 추가/갱신한 프레임 이름을 변경 내용에 명시한다 (Step 3 'History 표' 규칙 참조)
