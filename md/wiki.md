...
- **정책 코멘트 모드(기본)**: `points`의 좌표는 **코멘트 핀 위치(frame-relative)**, `no`는 **Description 통합 번호**(정책+`xlt` 마커를 y 오름차순으로 함께 매긴 번호 — 위 'xlt 마커도 통합 번호' 규칙). 이미지 ⓝ = Description 통합 번호 = XLT 표 No. 정책·`xlt` 마커 각 루트 핀마다 ⓝ를 찍어 Description·XLT No와 1:1로 맞춘다.
