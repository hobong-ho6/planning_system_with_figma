...
> ⛔ **ⓖ는 「하면 좋은 것」이 아니라 차단 단계다 (2026-09-15 실측).** LIAM은 `LOAD MESSAGE` 시점의 **스냅샷**을 저장한다 — Landpress를 아무리 고쳐도 LIAM을 다시 불러오지 않으면 **발송은 구 문구 그대로** 나간다. 실측에서 4건 모두 편집 화면을 열었을 때 옛 문구가 그대로 있었고, `LOAD MESSAGE` 이후에야 갱신됐다.
> - **언어 탭 수로 스냅샷 최신 여부를 판별할 수 있다** — LIAM은 **Landpress에서 `published: true`인 로케일만** 탭·`Message Lang`에 싣는다. 로케일을 추가·공개했는데 탭이 그대로면 **스냅샷이 낡은 것**이다(실측: 재동기화 후 2개 → 5개 언어).
> - **`Title`은 Landpress와 연동되지 않는다** — LIAM의 `Title`은 생성 시 입력한 **별도 필드**다. Landpress 항목명을 바꿔도 따라오지 않으므로 **Edit 화면에서 직접 고쳐야 한다**(실측: 항목명을 바꿨는데 LIAM은 구 명칭 유지).
