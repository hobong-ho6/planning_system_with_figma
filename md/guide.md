...
2. **치환자(Placeholder) 보존**
   ...
   - **⛔ 변수 + 통화/단위 표기는 한 칸 띄운다 (2026-07-28 확정, 5개 언어 공통)**: 치환자 뒤에 통화·단위가 붙으면 **`{0} JPYC`처럼 반드시 공백 1칸**을 넣는다 — `{0}JPYC`(붙임) 금지. **ja_JP·zh_TW도 동일하게 띄운다**(해당 언어 관용은 붙여쓰기이나, 변수 경계를 시각적으로 분리해 런타임 치환값이 단위에 달라붙어 보이는 것을 막기 위한 서비스 표기 정본). 용어집(v3.7~)에 `{0} JPYC`가 등재되어 있어 붙임 표기는 검증기가 P1로 잡는다. 예: `{0} JPYC 당첨됐어요!` / `{0} JPYCに当選しました！` / `You won {0} JPYC!` / `中獎 {0} JPYC！`
