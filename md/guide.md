...
2. **치환자(Placeholder) 보존**
   ...
   - **가변 값 변수화 제안**: 런타임에 바뀔 수 있는 값(요율 `10%`, 건수 `1건`, 금액 `100만엔`, 기간 `7일` 등)은 하드코딩하지 말고 `{0}` 변수로 처리하도록 **제안**한다. 실제 가변 여부는 비즈니스 판단이므로 자동 적용하지 말고 **사용자(PM) 확인 후 확정**한다. 수량 변수는 언어별 단/복수(plural)가 필요할 수 있다(`plurals` 시트 연동).
   - 예: 
