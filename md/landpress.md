...
5. **게시 후 태그**: **Claude가 직접 재배포**하고(2026-09-16 변경 · REST `PUT /api/sites/{siteId}` — 절차 `md/dropweb-guide.md` §8), `get_site`로 갱신을 확인한 뒤 `git tag guide-vN && git push --tags`. **라이브에 무엇이 있는지는 태그가 정본**이다(repo HEAD가 라이브보다 앞설 수 있다).
...
- **게시는 Claude가 직접 수행**한다(2026-09-16 변경) — 사용자 승인 후 REST로 재배포하고 `get_site`로 대조한다(절차 `md/dropweb-guide.md` §8). zip은 여전히 `dropweb/`에 남긴다.
...
| **갱신된 기획자 가이드 zip** | `dropweb/web3_planning_v*.zip` — 용어집 탭 4곳(이력 표·전체 용어 표·수치·임베드 JSON) 최신화 (§5-1 **필수**). 게시는 **Claude가 MCP로 직접**(승인 후) |
...
> ⛔ **쓰기는 Claude가 수행하는 것이 기본이다**(2026-09-15 사용자 확정) — 분량이 많다는 이유로 처음부터 「어느 쪽으로 할까요」라고 묻지 않는다. `lpc_safe_put.js`를 **사람이 붙여넣는 것은 대량 복구 같은 예외**다. 다만 **왕복이 많아 효율이 눈에 띄게 나빠지면 제안할 수 있다** — 실제 비용을 실측해 「N회 왕복이 더 필요하다」처럼 근거와 함께 제안하고, 사용자가 고르지 않으면 Claude가 계속 수행한다. 사람이 반드시 해야 하는 건 **로그인**(§10-1)뿐이다.

**⛔ 강제 산출물 2종 (prose 규칙만으로는 재발한다 — 2026-09-15 신설)**

| 언제 | 무엇 | 통과 기준 |
|---|---|---|
| **쓰기 전에** | `lpc_safe_put.js`의 **3중 안전장치 로직**으로 쓴다(read-modify-write · 덮어쓰기 가드 · 건별 재조회 대조) | 결과 `실패: 0` |
| **쓰기 직전·직후** | `python3 scripts/check_lpc_nulls.py` | **exit 0** (null 0 · 전건 게시) |

`lpc_safe_put.js`는 ① 현재 문서를 읽어 **콘텐츠 필드를 전부 실어** 보내고 ② 덮어쓰기 가드를 두고 ③ 건별로 **쓴 필드와 보존 필드를 각각 재조회 대조**한다. `check_lpc_nulls.py`는 **FE가 읽는 공개 조회 API**로 위키 정의 9개 컬렉션 × 4프로젝트 × 5개 언어를 훑어 `null`이 하나라도 있으면 exit 1이다(읽기 전용·인증 불필요).
