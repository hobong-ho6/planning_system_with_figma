...
- **Claude의 역할 = 산출물 전달(핸드오프)**: 검증 중 추가/수정이 필요한 용어를 발견하면 **갱신된 전체 JSON 산출물**을 만들어 사용자에게 전달한다. 사용자가 그 JSON을 Landpress CMS에 붙여넣어 반영한다. 반영 후 다음 `fetch_glossary` 조회 때 자동으로 최신본이 적용된다.
- **⚠️ Claude가 직접 쓰는 것도 기술적으로 가능하다**(§10). 다만 용어집은 **전 서비스 공용 라이브 데이터**이므로 **사용자가 그 작업을 명시적으로 지시한 경우에만** 직접 PUT한다. 기본값은 위의 산출물 전달이다.
...
> - **6단계(CMS 반영)** — 산출물만 만들고 끝내지 말고 **반영까지 확인**한다. 기본은 사용자가 붙여넣는 방식이고, 사용자가 지시하면 Claude가 직접 PUT할 수 있다(§10). 어느 쪽이든 **7단계 재조회 대조**로 반영을 확인한다. 로컬 `glossary.json`을 고쳐 "반영한 척" 금지.
...
□ 6. CMS 반영 — (기본) 사용자가 가이드 페이지/전달 JSON을 복사해 붙여넣기
       (사용자 지시 시) Claude가 CMS API로 직접 PUT (§10)
...
- **`landpress-content-v2.linecorp.com` 도메인의 탭이 열려 있어야 한다.** `fetch`가 그 페이지 컨텍스트에서 실행돼야 로그인 쿠키가 붙는다(터미널 `curl`로는 불가).
- **어느 페이지든 도메인만 같으면 된다** — 특정 프로젝트·컬렉션 화면일 필요가 없다. 실측상 dev 프로젝트 탭에서 prod 프로젝트 API를, 다른 프로젝트 탭에서 용어집 프로젝트 API를 정상 호출했다.
- 탭은 Claude가 직접 열 수 있다. **사용자는 CMS에 로그인만 되어 있으면 된다.**
- 작업이 끝나면 연 탭은 닫는다.
...
1. **사용자가 직접 지시했을 때만** PUT한다. 기본값은 §2의 산출물 전달이다.
