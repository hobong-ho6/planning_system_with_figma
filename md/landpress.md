...
**⛔ 0단계 — 최신 zip 판별 (2026-07-30 신설, 차단 단계)**

**파일명의 버전 숫자를 최신이라고 믿지 마라.** `dropweb/`에는 과거 zip이 모두 남아 있고, 파일명 번호와 실제 최신본이 어긋날 수 있다. 갱신 전 아래를 **반드시** 확인한다.
...
print('   내부 버전:', re.search(r'기획자 가이드 <b>(v\d)</b> \(([\d-]+)\)', s).groups())
print('   용어집 임베드:', re.search(r'&quot;version&quot;: &quot;([\d.]+)&quot;', s).group(1))"
done
```

- **판별 기준 = ⓐ 푸터의 내부 버전 표기 + ⓑ 용어집 임베드 버전 + ⓒ mtime**. 세 값이 가장 앞선 파일이 베이스다
- ⚠️ **실측 사고(2026-07-30)**: `web3_planning_v4.zip`의 mtime이 가장 최근이라 최신으로 착각해 그것을 베이스로 용어집을 갱신했다. 실제 최신은 **`v6.zip`(7/29)** 이었고, v6에는 이미 용어집 v3.9가 반영돼 있었다. 결과로 ⓐ "가이드가 4개 버전 밀렸다"는 **오진**을 사용자에게 보고했고, ⓑ v5·v6 변경(용어집 갱신 규칙 신설·FAQ 지원 등)이 빠진 zip을 전달했다(게시 전 발견해 회수). **mtime만 보면 안 되는 이유가 이것이다 — 구버전 파일을 나중에 만졌으면 mtime이 최신이 된다.**
- 파일명과 내용이 어긋난 zip을 발견하면 **삭제하거나 즉시 사용자에게 보고**한다(다음 작업에서 같은 오판을 유발한다)
- 새 버전 파일명은 **내부 최신 버전 + 1**로 만든다(파일명 최댓값 + 1이 아니다)
