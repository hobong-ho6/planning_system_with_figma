...
> ⛔ **쓰기는 Claude가 수행한다** — 분량이 많아 왕복이 늘어도 **사용자에게 콘솔 붙여넣기를 넘기지 않는다**(2026-09-15 사용자 확정). `lpc_safe_put.js`를 사람이 붙여넣는 것은 **대량 복구 같은 예외**이고, 평시에는 Claude가 그 로직 그대로 브라우저에서 실행한다. 사람이 해야 하는 건 **로그인**(§10-1)뿐이다.
