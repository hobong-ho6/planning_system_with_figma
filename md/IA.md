...
| 2026-09-14 | **주간 정기 점검 #7** — 프로덕션(비로그인 + **로그인**) + Beta(비로그인) + Unifi mini Beta(비로그인) | 인앱 브라우저(프로덕션 비로그인·Beta·mini) + Chrome(**프로덕션 로그인 세션 유지**) | 🔴 **오프라인 QR 결제 캐시백 캠페인 화면 신설**(`/pay/campaigns/offline` — ICB 파트너 가맹점·지역 탭 4종·9/10 공지), 🔴 **프로덕션 리워드 탭 스켈레톤 고착**(비로그인·로그인 모두 재현 / Beta 웹·mini는 정상 — #6과 환경이 역전됐다), 🔴 **KAIA 스테이킹 이율 4.2%→4.1% 하향**(홈·자산·`/reward/kaia` 3곳 일치), 🆕 **홈 「부스트 참여하고 USDT 받기」 배너 신설**, 🆕 **Apps에 「나의 관심 Apps」 섹션 신설·USDT 시세 출처 Bithumb 교체**, ❌ **swap AlphaSec 배너 소멸**, 🔴 **mini 홈에 카탈로그 복귀**(#6 「Web 허브와 완전 중복」 판정 번복), ✅ **mini Reward 스켈레톤 해소**, 🔴 **mini 「외부 게임 미션」 한국어 노출 재발** |
| 2026-09-21 | **주간 정기 점검 #8** — 프로덕션(비로그인 + **로그인**) + Beta(비로그인 + **로그인**) + Unifi mini Beta | 인앱 브라우저 375px(비로그인 전 범위) + **세션 중 사용자가 로그인 제공** → Chrome(프로덕션 로그인)·**인앱(Beta 로그인 — Chrome은 Beta 도메인 차단, 인앱은 가능하다는 것을 이번에 확인)** | 🔴 **스켈레톤이 「비로그인 해소 / 로그인 고착」으로 갈렸다**(#7 최우선 이월 — 조건만 좁혀졌고 미해결 · 401은 원인이 아님을 대조로 확인), 🔴 **KAIA 이율 표기 3중 불일치**(ko 4.2% / en 4.1% / Beta 0% — #7 「4.1% 하향 3곳 일치」 판정 정정 · **XLT 확인 대상**), ✅ **캐시백 조회 동작 확인**(별도 화면이 아니라 인라인 「My Cashback Status」 — 이월 해소 · 승인 대기 ⑫ 재검토), ✅ **Beta 로그인 영역 첫 실측**(이월 3번 해소 · 비상장 토큰 4종 재확인), ✅ **Apps 「구조 변경 3건」은 변경이 아니라 로그인×통화 축 변형으로 판명**(#7 기재 2건 정정 — 「오늘도 환생2」=Endless Frontier2 · 시세 출처=통화 연동 / 실제 변경은 **26→25 Apps**뿐), 🆕 **NEXT Bay 미션 프로덕션 승격**, 🔴 **Beta K-Pick 허브가 풀 카탈로그로 전면 개편**(#7 「허브로 축소」 번복) · **mini 홈과 다시 완전 동일**(3주 연속 번복), 🔴 **프로덕션 `/benefits` → 홈 리다이렉트**, 🔴 **mini GNB 3탭**(기재 5탭과 불일치), 🆕 **`/apps/market` Ranking 탭 신설**, 🔴 **KAIA CR STAGE 3 전환**, ⚠️ **K-Pick 가격 표기 이상**(¥25,294,160), 🔴 mini 「외부 게임 미션」 한국어 **2주 연속 재현** |
...
| 예치·이자 | 제공 — **BASIC 연 3% / PLUS 연 7%**(2026-08-06 발효, 구 5%) + Boost 최대 3% = **최대 연 10%** | **이율 정보 전부 제거** (자산·자산상세) | ⚠️ 위키 스펙은 "미제공(예치금·이자 노출 불가)" — 그러나 **Beta mini Reward 상단에 JPYC 이자 배너가 4주 연속 실측**(2026-08-03·08-10·09-03). 🆕 **수치가 「최대 연 5%」 → 「最大年2%」로 하향**(2026-09-03). 🔴 **2026-09-07 mini 자산 탭(`/my`)의 JPYC 카드에도 「最大年2%の利息を受け取る」가 노출**돼 충돌이 리워드 탭 밖으로 확대됐다 → 확인 필요(§4). ⚠️ **2026-09-14 mini Reward 탭에서 이자 배너가 보이지 않았다**(비로그인 기준 — 자리에 JPYC 구매 가이드 배너) → 정책 반영인지 비로그인 한정인지 **로그인 재확인 필요** |
...
| 결제 | 지갑 연결 + 🆕 **오프라인 QR 결제(Unifi Pay)** — 2026-09-03 실측 · 사전 충전 잔액에서 차감 · 🆕 **2026-09-07 스캔 화면(`/pay/qr/mpm`)·로그인 홈 헤더 [pay] 버튼 실측** · 🔴 **2026-09-14 오프라인 캐시백 캠페인 화면 신설(`/pay/campaigns/offline` — ICB 파트너 가맹점)**(§2-9) | 지갑 연결 | **LINE IAP** (지갑 연결 없음) + **JPYC 결제**(2026-07-10 공지 도입) |
...
| `/pay/campaigns/offline` | 🆕 오프라인 결제 캐시백 캠페인 | 🟢 | 🆕 **2026-09-14 신설**. 비로그인 홈 배너 「Pay with QR, get 10% back」로 진입 · 프로덕션·Beta 공통 · **로그인 홈에는 배너가 없다** |
| `/pay/campaigns/offline?campaignStatus=view` | 🆕 내 캐시백 조회 | 🔴 | 🆕 비로그인에서 [View my cashback] → `/auth/sign-in?returnUrl=…campaignStatus%3Dview`. ⚠️ **로그인 상태에서 버튼·쿼리 모두 무반응**(화면 변화 없음) — 미실측·FE 확인 대상(§4) |
...
| **Web × KR IP × 로그인** | ✅ **실측 완료 (2026-08-10 · 09-07 · 09-14 프로덕션 로그인 전체 순회)** — `/my`·`/setting`·`/notification`·`/my/token/transaction`·`/interest/usdt`·`/transfer`·`/deposit`·`/apps/trade/swap`·`/apps/my-page/nfts` | Chrome (프로덕션+Beta 양쪽 세션) |
...
- 🔴 **2026-09-21 라우트 변경 — 프로덕션 `/benefits`가 홈(`/`)으로 리다이렉트된다**: `location.href` 실측으로 `https://www.unifi.me/`로 치환됨을 확인했다(비로그인·KR IP).
  #7까지 이 주소는 **K-Pick 허브(배너 3종)** 화면이었다. **하위 `/benefits/k-pick/{beauty|shopping|pop}`는 여전히 열려 있고**(탭 3종 노출·KR IP 카탈로그 0건),
  `/benefits/daily-mission`도 정상이다 — 즉 **허브 진입점만 닫혔다**. Beta는 `/benefits`가 그대로 유지되므로 **프로덕션 한정 변경**이다.
  ⚠️ K-Pick GNB 승격 추적(§4)과 반대 방향의 움직임이므로 다음 회차에 재확인한다.
- 🆕 **2026-09-14 신설 라우트**: 결제 캠페인 **`/pay/campaigns/offline`**(비로그인 열람) · 내 캐시백 **`/pay/campaigns/offline?campaignStatus=view`**(로그인 필수 · 무반응 — §4). 캠페인 배너 링크에 `utm_*`·`referral_code` 파라미터가 붙는다.
- 🆕 **홈 헤더 아이콘의 GA 이벤트 어휘 실측 (2026-09-14)**: 좌→우 **`click_home_pay_qr`**(→ `/pay/qr/mpm`) · **`click_address`**(→ `/deposit`) · **`click_gnb_noticenter`**(→ `/notification`). ⚠️ #6에 「[QR] = 입금」으로 적은 두 번째 아이콘의 정본 어휘는 **「주소(address)」**다.
...
home_pay_campaign_01 ⚠️잠정        🆕 **오프라인 결제 캐시백 캠페인 배너 (2026-09-14 신설 · 비로그인 전용)** — "Pay with QR, get 10% back / Instant USDT cashback in Korea"
                                  → `/pay/campaigns/offline`(§2-9) · 프로덕션·Beta 공통 · **로그인 홈에는 없다**
home_benefit_boost_01             🆕 **부스트 배너 (2026-09-14 신설 · 프로덕션·Beta 공통)** — 「부스트 참여하고 USDT 받기 / 최대 3% 추가 이자 받기」 → `/reward/usdt`
home_main_01                      홈 메인 — 자산 요약 카드(USDT/JPYC/IDRP 탭·입금하기·플러스 모드 배너·누적 이자)
                                  └ 🔴 **로그인 홈 전면 개편 (2026-09-07 프로덕션 로그인 실측)** — 비로그인 홈과 구성이 통째로 다르다
                                     · **헤더 아이콘 3종**: **[pay]**(`click_home_pay_qr` → `/pay/qr/mpm` QR 결제 · **로그인 전용** — #5 이월 항목 해소) · **[주소]**(`click_address` → `/deposit` 입금 · 🆕 2026-09-14 GA 어휘 실측으로 「QR」→「주소」 정정) · **[알림]**(`click_gnb_noticenter` · 레드닷)
                                     · 🆕 **토큰 3카드 자체가 토큰 상세 링크다 (2026-09-14)** — 카드 전체가 `/my/token/{컨트랙트주소}`로 이동한다(카드 내 [채우기]는 별도)
...
home_benefit_together_01          🆕 함께하면 더 큰 혜택, KAIA & USDT (Better Together — /reward/kaia 진입 배너 · 2026-07-30 신설)
                                  └ 🔴 **부제 수치가 최대 4.2% → 「최대 4.1% 보상에 특별 혜택까지!」로 하향됐다 (2026-09-14 프로덕션 로그인·비로그인 공통)**
                                     — 내 자산 KAIA 카드·`/reward/kaia` 본문과도 **4.1%로 일치**한다(§2-2-1·§2-3). 수치를 상수로 인용하지 않는다
                                     🔴 **2026-09-21 정정 — 「3곳 일치」가 깨졌다. 언어별로 갈린다**: 한국어(로그인) 3곳 전부 **「최대 4.2%」** · 영문(비로그인) **「Up to 4.1%」** · Beta 영문 **「Up to 0%」**.
                                     ⛔ **이율은 언어·환경마다 다른 값이 나오므로 어느 한 값을 정본으로 인용하지 않는다** — XLT 확인 대상(§4)
...
reward_main_01                    Rewards 메인 (리워드 USDT·럭키볼 개수 요약)
                                  └ 🔴 **2026-09-21 — 스켈레톤이 「비로그인 해소 / 로그인 고착」으로 갈렸다** (#7은 양쪽 모두 고착)
                                     · **비로그인 ✅ 정상**: 카운트다운 「22:37:39 remaining」 · Reward 0 USDT · Lucky ball 0 · 출석 5일 진행도 · 게임 6종 `0/5~0/7` · Explore Apps 전부 렌더
                                     · **로그인 ❌ 고착**: Chrome 로그인 세션으로 재진입해도 **20초 후 대시 블록 31개**, 출석 섹션 미렌더(2회 재현)
                                     · ⚠️ 콘솔 `ACCESS_DENIED`(401)는 **비로그인에서도 찍히는데 화면은 정상**이다 — 401 자체는 고착의 원인이 아니다(#7의 진단 근거를 정정한다)
                                     · ⛔ **최우선 이월 유지 — 단 조건이 「로그인 한정」으로 좁혀졌다.** FE 확인 권장
                                  └ (이력 보존) 🔴 **2026-09-14 프로덕션 리워드 탭 스켈레톤 고착 — 환경이 역전됐다**
                                     · 인앱 브라우저(비로그인)·Chrome(**로그인**) 양쪽에서 **15초 후에도 대시 블록 33개**로 고착(3회 재현)
                                     · 렌더되는 것은 헤더 「Rewards」·「미션 / 일일 미션하고 럭키볼로 최대 500 USDT 뽑기」·「Apps 둘러보기」뿐
                                     · 콘솔 **`ACCESS_DENIED`(401)** · `api-reward.unifi.me/v1/mission-users` 401 (`/v1/token-info/country`는 200)
                                     · ✅ **Beta 웹·Beta mini 리워드 탭은 정상 렌더** — #6(프로덕션 정상·Beta mini 고착)과 **정반대**
                                     · ⛔ 가려진 영역: 출석 체크 진행도·게임 미션 6종·럭키볼 요약 → **이 회차 재확인 불가**. **FE 확인 권장 · 최우선 이월**(§4)
...
reward_mission_market_01          🆕 **NEXT Bay 게임 마켓플레이스 미션 (2026-09-03 Beta 실측 · 🔴 2026-09-21 프로덕션 승격)**
                                  └ ⛔ **「프로덕션 미노출」 기재 정정 (2026-09-21)** — 프로덕션 리워드 탭 **최상단 배너**로 노출된다(비로그인 실측).
                                     영문 문구 **"Complete the special mission on NEXT Bay game marketplace / and get up to 100 USDT"** · Beta·mini에도 동일 배너
...
reward_boost_usdt_01 ⚠️잠정        USDT Reward 탭 (/reward/usdt — 구 /boost/kaia)
                                  ...
                                  └ 🆕 **한국어 전문 실측 (2026-09-14 로그인)**: 「부스트 혜택은 상시로 자동 적용됩니다 / 별도의 만기나 유지 기간 종료 없이 상시 운영돼요」 ·
                                     「Tier는 잔고에 따라 매일 갱신됩니다 — 매일 이자 지급 시점(00:00 UTC+0)의 잔고에 따라 다음 날 Tier가 즉시 상/하향」 ·
                                     「KAIA 보유량이 Tier 1 미만으로 떨어지면 당일 이자는 미지급되나, 추가 입금하여 조건을 채우면 해당일부터 즉시 다시 지급」
...
reward_staking_kaia_01 ⚠️잠정      KAIA Reward 탭 (/reward/kaia — **KAIA Dual Rewards: Base + Special**)
                                  └ **Staking Rewards / KAIA Staking**: 위임으로 연 최대 **4.1%**(🔴 2026-09-14 **4.2% → 4.1% 하향** — 홈 배너·내 자산 카드와 일치) · Accrued Reward · **Delegate 버튼** · Delegatable KAIA
                                  └ **Special Contribution Rewards**(= CR): KAIA 위임 + USDT 보유 시 10 USDT당 최대 0.449999 KAIA · Claimable reward · USDT principal
                                  └ **Mission Check Period**: 🔴 **STAGE 3 전환 (2026-09-21 실측)** — **1R 9.21~10.1 진행 중** / 2R 10.1~10.11 예정 / 3R 10.11~10.21 예정 · 전체 보기
                                     ※ STAGE 1은 1R 7.23~8.2 / 2R 8.2~8.12 / 3R 8.12~8.22 · STAGE 2는 1R 8.22~9.1 / 2R 9.1~9.11 / 3R 9.11~9.21이었다 — **약 10일 회차 3개가 한 STAGE를 이룬다**
                                     ※ 이율 **최대 연 4.1%**·10 USDT당 최대 **0.449999 KAIA**·7일 쿨다운·자동 재위임은 2026-09-21에도 동일(정책 변동 없음)
                                  └ 🆕 **2026-09-21 영문 유의사항 전문 실측** — CR은 **6개월 분할 지급**이며 **매월 [Claim Rewards]를 직접 눌러야** 수령된다(자동 입금 아님) ·
                                     조건 미충족으로 미분배된 CR은 **약 90일(3 Epochs) 후 영구 소각** · 참여 총액이 **상한 5,000만 USDT**를 넘으면 보상 비율이 비례 축소(표시 이율은 확정값이 아님) ·
                                     미션 기간 중 부분 출금·언스테이킹 시 그 회차 기여도는 **잔여 유지 자산(최소 잔고)** 기준으로 인정
...
asset_main_01                     내 자산 메인 — 나의 총 자산·액션 4종·토큰 목록(USDT/KAIA/JPYC/IDRP)·보유 NFT
                                  ...
                                  └ 🆕 **프로덕션 카드 유도 문구 확정 (2026-09-07 로그인 실측)**: USDT "**Hold USDT in Plus Mode and Earn 7% annual interest with no conditions**"(→ `/plus/usdt`) ·
                                     KAIA "**Stake KAIA and Earn up to 4.2% annual interest**"(→ `/reward/kaia` · ~~2026-09-14 한국어 4.1%로 하향~~ · 🔴 **2026-09-21 한국어 실측은 「KAIA 스테이킹하고 최대 연 4.2% 이자 받으세요」 — 4.2%다**) ·
                                     **JPYC 카드에 「최대 연 2%」 배지**(#5까지 Beta·mini 전용이던 표기가 프로덕션 반영 · 2026-09-21 한국어 라벨 확인) · IDRP는 문구 없음
                                  └ 🆕 **2026-09-21 로그인 실측(한국어·KRW · Preferred Stable = JPYC)**: 총자산 우측 **거래내역** · 액션 **보내기·채우기·교환하기·은행출금** 4종 · 카드 순서 **USDT → KAIA → JPYC → IDRP** ·
                                     USDT 카드 「연 3%」 + 「플러스 모드에 USDT 보관하고 조건 없이 연 7% 이자 받으세요」 · **보유 NFT 0** ·
                                     ⚠️ **홈의 토큰 카드 순서(JPYC→USDT→IDRP)와 내 자산의 순서(USDT→KAIA→JPYC→IDRP)가 다르다** — Preferred Stable을 따르는 것은 **홈**이다(§0-0-1 축 ①-b 정밀화)
...
asset_interest_usdt_01 ⚠️잠정      🆕 **나의 USDT 누적 이자 (`/interest/usdt`)** — 2026-08-10 신규 실측 · IA에 없던 라우트
                                  ...
                                  └ 🆕 **빈 상태 실측 (2026-09-14 · 이자 수령 이력 0건)**: 「받은 이자 내역이 없습니다 / USDT를 입금하고 이자를 받아보세요.」 + CTA **[USDT 입금하기]**
                                     ※ 누적 이자가 0에 가까우면 내 자산 USDT 카드의 「Total Interest」 인라인 표기도 보이지 않는다(진입점은 화면 자체 URL)
...
asset_deposit_01                  입금하기 (/deposit — 네트워크(KAIA)·내 지갑주소 복사·카테고리 탭(스테이블 코인/다른 토큰)·토큰별(USDT/JPYC/IDRP) 안내)
                                  ...
                                  └ 🔴 **2026-09-14 JPYC 1단계 문구가 다시 바뀌었다** — ① 「**JPYC를 Unifi 지갑으로 전송하세요** / Unifi에서 내 지갑 주소를 복사한 후, 보유 중인 JPYC를 해당 주소로 입금하세요」
                                     + **[JPYC 입금 가이드 보기]** ② 「지갑에 입금된 JPYC를 확인하세요」. **#6의 외부 구매처(jpyc.co.jp) 유도 문구가 사라지고 「지갑 전송 안내」로 되돌아갔다**
                                     (2단계 구조·[가이드] 버튼은 유지 · 네트워크 안내 「KAIA 네트워크의 모든 토큰은 동일한 지갑주소를 사용합니다」 유지)
...
apps_main_01                      Apps 메인 — Reward 서브탭 (/apps · **비로그인 열람 가능**)
                                  ...
                                  └ 🆕 **2026-09-14 구조 변경 3건**
                                     · 🆕 **「나의 관심 Apps」 섹션 신설**(게임 프로모션 캐러셀 바로 아래 · [전체보기] 동반) — IA에 없던 영역
                                     · 게임 프로모션 캐러셀이 **5종 → 3종**으로 축소(🆕 **「오늘도 환생2」** 신규 / ❌ Endless Frontier2·LORDNINE·Seal M 미노출 · 잔류 Siege Of Titans·LEGEND WAR)
                                     · **USDT 시세 출처가 CoinMarketCap → Bithumb으로 교체**(KAIA는 CoinMarketCap 유지) — 표기는 통화 설정을 따른다(KRW 계정에서 ₩)
                                  └ ✅ **카테고리 7종 2주 연속 확인 (2026-09-14)** — AI·CONTENT·DePIN·GAME·Payment·SOCIAL·ETC. `SocialFi` 소멸이 일시 현상이 아니라 **카테고리 폐지**로 판정한다(#6 추적 항목 종결)
                                  └ 수혜자 수·시세 위젯은 **매 점검 변동**(2026-09-21: 9,023,036명 / 2026-09-14: 9,022,377명 · 1 KAIA=₩39 · 1 USDT=₩1,360 / 2026-09-07: 9,021,821명) — 변경 판단 대상 아님
                                  └ ✅ **2026-09-21 — #7의 「구조 변경 3건」은 변경이 아니라 「로그인 × 통화·언어」 축의 변형이었다 (같은 날 비로그인·로그인 양쪽 실측으로 확정)**

                                     | 요소 | **비로그인**(영문·USD) | **로그인**(한국어·KRW) |
                                     |---|---|---|
                                     | 게임 프로모션 캐러셀 | **5종**(LEGEND WAR·Endless Frontier2·LORDNINE·Siege Of Titans·Seal M) | **3종**(LEGEND WAR·오늘도 환생2·Siege Of Titans) |
                                     | 「나의 관심 Apps」 | ❌ 없음 | ✅ **있음**(캐러셀 바로 아래 · [전체보기]) — **로그인 전용 섹션** |
                                     | Editor's Pick | ✅ 있음 | ✅ 있음(**「에디터's Pick」**) — 관심 Apps가 **대체하는 것이 아니라 추가**된다 |
                                     | 시세 출처 | CoinMarketCap **USDT $0.99** · Binance **KAIA $0.03** | Bithumb **USDT ₩1,366** · CoinMarketCap **KAIA ₩42** |
                                     | 리워드 미션 | 미션 3종만 | 미션 3종 + **「나의 USDT/KAIA 리워드」 잔액 줄** |

                                     · ⛔ **#7 기재 정정 — 「오늘도 환생2 신규 / Endless Frontier2 미노출」은 오기다.** 같은 게임의 언어별 표기다
                                       (en "Endless Frontier2 / Rebirth today for Epic Rewards!" ↔ ko "오늘도 환생2 / 오늘 바로 환생하고 epic 보상을 받으세요!" — 문구가 1:1 대응한다)
                                     · ⛔ **#7 기재 정정 — 「USDT 시세 출처가 Bithumb으로 교체」도 변경이 아니다.** 출처는 **통화 설정에 연동**된다(KRW→Bithumb / USD→CoinMarketCap)
                                     · 🔴 **실제 변경은 Apps 수뿐이다 — 26개 → 25개**(비로그인·로그인 양쪽 동일하게 25개로 확인)
                                  └ ✅ **카테고리 7종 3주 연속 확인 (2026-09-21)** — `SocialFi` 폐지 판정 유지
apps_market_01                    Apps 마켓 서브탭 (/apps/market — Buy/Sell · Drops: Live & Upcoming / Past / Now · NFT 드롭 카드(가격 KAIA·수량·판매율))
                                  └ 🆕 **「Ranking」 서브탭 신설 (2026-09-21 실측)** — 상단 탭이 **Buy · Sell · Ranking** 3종으로 늘었다(종전 Buy/Sell 2종).
                                     구성: 기간 필터 **「Daily」** + 기준일(2026.09.21) + 툴팁 버튼 + 랭킹 목록 **7건**(BlueSL · BlueSL2nd · Dr.Paws · Slime Miner Gold Gears · "Nani-kiru" Membership Card · Chiwat's Golden Pack · Bubbleheroes: legendary hero DingDing)
                                     ⚠️ 비로그인이라 **순위 값이 전부 `-`**로 비어 있다 — 정렬 기준(거래량·거래액)·기간 필터 나머지 옵션은 **미확인**(§4)
                                  └ Drops는 동일 — Dr.Paws **500 KAIA · 1,000개 · 판매율 0.8%**(2026-09-14와 같은 값)
apps_mypage_nft_01                나의 NFTs (/apps/my-page/nfts — 탭 3종: 전체·판매중·거래내역 / 빈 상태 "지갑에 보유하고 있는 NFT가 없어요")
                                  └ 🆕 **2026-09-07 영문 빈 상태 실측**: "**No NFTs / You don't have any NFTs in your wallet.**"
                                  └ ⛔ **2026-09-14 기재 정정** — 보유 0건에서도 **탭 3종이 노출된다**(「전체 0 · 판매중 0 · 거래내역」). #6의 "탭 미노출" 기재는 오기이므로 폐기하고, 빈 상태는 **탭 + 「NFT 없음 / 지갑에 보유하고 있는 NFT가 없어요」** 구성으로 확정한다
...
apps_trade_swap_01                교환하기 (/apps/trade/swap — From/To 토큰 선택·Max·스왑 방향 전환 버튼·교환 · JP 미제공·mini 미제공)
                                  ...
                                  └ ❌ **2026-09-14 AlphaSec 배너 자체가 사라졌다** — 화면은 From/To·잔고·[최대]·방향 전환·[교환]만 남았다. 프로모션 종료로 보이며, 부활 시 새 항목으로 기록한다
...
my_setting_preferred_stable_01 ⚠️잠정  🔴 **화면 표시 — 「선호 스테이블 코인」(Preferred Stable) 신설 (2026-09-07 실측 · 🆕 2026-09-14 한국어 라벨 확정 · 값 JPYC/USDT/IDRP)**
...
kpick_main_01                     K-Pick 탭 메인 (**Beta 라우트 `/benefits`** · 헤더 좌 "K-Pick" / 헤더 우 **MY쇼핑 링크 + noti 벨**)
                                  ...
                                  └ 🔴 **프로덕션 `/benefits`는 KR IP에서 축소판이다 (2026-09-03 신규 실측)** — 배너 3종만 노출
                                     ...
                                     **카테고리 4아이콘·바우처 12종·스킨부스터·K-컬처 전부 미노출** → `kpick_kr_block_01`(KR IP 제한)은 **프로덕션에서 유효**하며 폐기 대상이 아니다
                                     🔴 **2026-09-21 — 프로덕션 `/benefits`는 이제 홈(`/`)으로 리다이렉트된다**(배너 3종 화면 자체가 사라졌다 · §0-3). 하위 `k-pick/*`는 개방 유지
                                  └ 🔴 **2026-09-21 Beta 허브 전면 개편 — 「허브로 축소」(#6·#7) 판정을 번복한다.** `/benefits`가 다시 **풀 카탈로그 화면**이 됐고, 구성이 K-Pick 카테고리 체계와 함께 바뀌었다:
                                     · 히어로 **"Korea Trip, Where to Start? / OLIVE YOUNG, DAISO & more — Buy early, get up to 15%"** + 브랜드 칩 4종(Olive Young·Daiso·CU·**emart24** · 각 15%)
                                     · 🆕 **카테고리 아이콘이 5종으로 교체** — **Clinics · K-Beauty · Esthetic · Tours & Tickets · Activities**
                                       ⛔ 종전 4아이콘(K-Beauty·K-Shopping·K-Culture·My Reservation)과 **분류 체계가 다르다** — 라우트(`k-pick/{beauty|shopping|pop}`)는 3종 그대로이므로 **허브 아이콘 ↔ 라우트가 1:1이 아니다**(어휘 결정 시 주의)
                                     · 🆕 **「Popular Korean vouchers — 1,000 JPY OFF now!」 쿠폰 프로모션 블록** + 바우처 12종(정가 병기 「Regular price」)
                                     · 🆕 **클리닉이 허브로 올라왔다** — 「Clinics Offering 10% or More Cashback」 10곳 + **「Clinic with the highest number of reviews」 TOP 18 랭킹**(리뷰 수·평점·캐시백 %)
                                     · 🆕 **「Popular Korean treatments by skin concern」** 큐레이션 3건(쥬베룩=밀리클리닉 도산 / 리쥬란=청담봄온 / 포텐자=델픽) — #7까지 mini 홈에만 있던 블록이 **Beta 웹 허브에도 붙었다**
                                     · 🆕 **「Trending K-Beauty Care」**(K-Beauty/Esthetic 탭) · **「Popular Must-Visit Itinerary in Korea」**(Activities/Tours & Tickets 탭) · **「K-POP Idol Tour」** 섹션 신설 — 전부 환원 7%
                                     ※ 허브의 **「15%」는 할인 5% + 캐시백 10%의 합산 표기**다(카테고리 목록은 「5% OFF」+「10% cashback」으로 분리 표기) — **환원율 상향이 아니다**
                                  └ 🔴 **2026-09-21 mini 홈(`/benefits-mini`) = Beta 웹 허브와 다시 완전 동일** — 문구·상품·클리닉 랭킹까지 일치한다.
                                     #6 「완전 중복」 → #7 「mini에만 카탈로그」 → #8 「다시 완전 중복」으로 **3주 연속 판정이 뒤집혔다**.
                                     ➡️ **구성 스냅샷으로 어휘를 정하면 매주 무너진다** — 「허브 + (환경별) 카탈로그」 구조로 정의하자는 #7 권고의 근거가 강화됐다(§4)
                                     ※ 타이틀은 환경별로 다르다: mini **"Unifi | 渡韓のおトクサービス"** / Beta 웹 **"Unifi | A stablecoin wallet designed for earning"**
kpick_category_01                 🆕 **K-Pick 카테고리 목록 (2026-09-03 라우트 신설 · 2026-09-07 카탈로그 이관으로 정본 화면이 됐다)**
                                  ...
                                  └ 라벨 K-美容 / K-買い物 / K-カルチャー(ja UI) · 🆕 **영문 UI 라벨 확정 (2026-09-14)**: **K-Beauty / K-Shopping / K-Culture**(프로덕션·Beta·mini 공통) ·
                                     허브의 카테고리 4아이콘은 **K-Beauty · K-Shopping · K-Culture · My Reservation** · ⚠️ **「내 예약」은 여전히 `href="#"`** — 라우트 없음
                                  └ 🆕 **mini 카테고리 목록에는 헤더 타이틀 「K-Pick」이 있다 (2026-09-14)** — Beta Web 목록 헤더는 [back]·「My Shopping」뿐이다(같은 화면의 환경 차이)
                                  └ 🔴 **K-美容에 「クリニック」 섹션 신설 (2026-09-07 · Beta Web·mini 공통)** — 이 저장소 clinic 위키 작업 대상 화면이 라이브에 올라왔다
                                     ...
                                     · 🆕 **2026-09-14 재실측**: 클리닉 **18곳**(#6 19곳) · 필터 영문 라벨 **All / Dermatology & Plastic Surge(ry) / Dentistry / Ophthalmology** ·
                                       **클리닉 캐시백은 카드별로 다르다**(티아나 15% · 원데이치과 10% · 레디피부과 8% · 강남헤라 5% · 힐링안과 15%) — **카테고리 균일 7%와 별개 축**이다
                                     · 🆕 **2026-09-21 재실측**: 클리닉 **18곳 유지** · 필터 4종 유지 · 캐시백 5~15% 카드별 차등 유지 ·
                                       🆕 카드에 **시술명 태그**(Juvelook · Rejuran · Potenza · Laminate Veneers · Dental Implant)와 **강점 불릿 4종**(1인 진료실 · 의료진 8명+ · 강남역 인접 · 정품 정량 원칙 등)이 함께 노출된다 · **[Inquire]** 라벨(영문)
                                     · ⛔ **어휘 미확정 — 부여 금지**(§4 승인 대기 ⑩)
                                  └ 🔴 **2026-09-21 카테고리 목록 개편 (Beta·mini 공통)**
                                     · **K-Beauty(`/k-pick/beauty`)가 「Korean Beauty & Aesthetic Care」로 확장** — 헤어·마사지·스파·퍼스널컬러 등 **에스테틱 서비스 20종**(15~25% OFF · 환원 7%)이 앞에 오고 **클리닉 섹션이 그 아래**에 붙는다
                                     · **K-Shopping(`/k-pick/shopping`) = 「Voucher」 화면 · 환원 10%** — 바우처 **13종**(12종 + 🆕 **TOM N TOMS Iced Americano T**). #7에서 **mini 전용 신규**로 기록한 상품이 **Beta 웹에도 올라왔다**
                                       ※ 「이마트」로 적었던 브랜드의 정본 표기는 **emart24**다(영문 UI 실측) · 환원 **10% 균일**(#7 「올리브영 1만원권만 5%」 예외는 이번 회차에 관측되지 않음)
                                     · **K-Culture(`/k-pick/pop`) 22건·환원 7% 유지** · 태그 배지의 **ja(ソウル·京畿道·仁川) + en(DMZ·KPOP·Show Ticket) 혼재도 그대로 재현**(§4 추적 유지)
                                       🆕 신규 관측: 롯데월드 종일권 · 스파렉스 동묘 찜질방 · Ktown4u 레코딩 MV 패키지 · 남이섬&쁘띠프랑스 버스투어 · 미쉐린 1스타 셰프 쿠킹클래스 · DMZ 투어
                                  └ ⚠️ 🔴 **2026-09-21 가격 표기 이상 — 허브와 카테고리의 값이 서로 다르다 (FE·데이터 확인 권장 · §4)**
                                     · **동일 상품·동일 할인율인데 가격이 다르다**: Juno Hair 홍대 허브 **¥26,086** ↔ 카테고리 **¥18,941** / Hoso 도산청담 허브 **¥3,030** ↔ 카테고리 **¥1,265** / Marzia 힐링스파 허브 **¥35,435** ↔ 카테고리 **¥16,106**
                                     · **명백한 이상치**: 마리엠 헤어&헤드스파가 허브에서 **¥25,294,160**(2천5백만 엔)으로 나온다 — 카테고리에서는 **¥21,821**. **Beta 웹·mini 양쪽에서 동일 재현**(환경 문제가 아니라 데이터·바인딩 문제)
                                     · K-Pop 걸그룹 스타일링 촬영도 허브 **¥259,854** ↔ 카테고리 **¥255,503**으로 다르고, 같은 목록의 유사 상품(¥1,265~¥28,303) 대비 **한 자릿수 이상 크다**
                                  ...
                                  └ 🆕 **2026-09-14 카탈로그 실측치**: K-Beauty **20건**(+클리닉 18곳) · K-Shopping **바우처 12종**(올리브영 3·다이소 3·CU 2·이마트 4 — #6과 동일 구성) · K-Culture **22건**
                                  └ 🆕 **K-Culture 상품 카드에 태그 배지가 붙었다 (2026-09-14)** — 예: 「ソウル」·「仁川」·「Show Ticket」·「Martial Arts」·「KPOP」·「Dance class」·「Aquarium」.
                                     ⚠️ **일본어(지역)와 영문(장르)이 한 카드 안에 혼재**한다 — 표기 정책 확인 대상(§4). 🆕 신규 상품 **코엑스 아쿠아리움 입장권**
...
/benefits-mini                    mini 홈 — 🔴 **2026-09-03 전면 개편: K-Pick형 커머스 화면으로 통째로 교체됐다**
                                  ...
                                  └ 🔴 **2026-09-14 카탈로그가 mini 홈으로 되돌아왔다 — #6 「완전 중복」 판정 번복**
                                     현재 mini 홈 = 허브(JPYC 배너·15% 환원 배너·카테고리 4아이콘) **+ 🆕 「The most affordable way to plan your Korea trip」 › 「Must-have vouchers for your Korea trip」 바우처 12종**
                                     **+ 큐레이션 3건**(클리닉·리쥬란·포텐자) **+ K-뷰티 상품**(Good Morning Yakson 등). 🆕 신규 상품 **TOM N TOMS Iced Americano**(환원 5% · `test.guidekim.me`)
                                     ➡️ **Beta Web `/benefits` 허브에는 카탈로그가 없다** — 두 화면은 다시 달라졌고, 중복 판단은 「허브는 같고 홈에만 카탈로그가 붙는다」로 갱신해야 한다(§4)
                                  └ 🆕 **로그인 상태 실측(2026-09-07)**: 카테고리 행 옆에 **「進行中のご予約があります」 툴팁 + マイ予約** 노출(비로그인에는 없음)
                                     · ⚠️ **2026-09-14 비로그인에서도 같은 툴팁(「You have a reservation in progress」 + My Reservation)이 노출됐다** — 로그인 한정 기재와 상충. 조건 재확인 필요(§4)
...
(2026-08-03 시점의 구 구성 — 이력 보존)
                                  ...
                                  └ ✅ **2026-09-14 mini Reward 스켈레톤 해소** — 비로그인에서 대시 블록 0개, Special Missions·Mission·Daily check-in·Game mission 전부 정상 렌더.
                                     ⚠️ 단 **JPYC 이자 배너(「最大年2%」)는 이번 회차에 보이지 않았다**(자리에 JPYC 구매 가이드 배너) — 정책 반영인지 비로그인 한정인지 로그인 재확인 필요(§4)
...
/benefits-mini/daily-mission      mini Reward 탭 — "Rewards" 헤더 (2026-08-03 상세 실측)
                                  ...
                                  └ ~~**Game mission** — "게임 미션 완료하고 JPYC 받기" / 항목 **"외부 게임 미션"**(0/2)~~
                                     ...
                                     → 🔴 **2026-09-14 섹션 부활 + XLT 누락 재발**: "Game mission / Complete game missions to earn JPYC" 아래 항목이 **「외부 게임 미션」 한국어 그대로** 노출되고
                                       "Before proceeding with mission 0/2"가 붙는다. 영문 UI인데 항목명만 한국어다 — **XLT 키 누락 의심 · #5에서 종결한 건의 재발**(§4)
                                     → 🔴 **2026-09-21 2주 연속 재현** — 문구·구성 동일(「외부 게임 미션」 + "Before proceeding with mission 0/2"). **일시 현상이 아니다 — XLT 키 확인 필요**(§4)
                                  └ 🆕 **2026-09-21 Special Missions 구성 실측** — 「Special Missions」 아래 ⓐ **NEXT Bay 배너**("Complete the special mission on NEXT Bay game marketplace / and get up to 100 USDT")
                                     ⓑ **"Unlimited missions and Up to ¥30,000 instantly / Game / Ad / Survey missions"**(SkyFlag 계열) 2블록으로 구성된다. #6에서 제목·본문만 렌더됐던 영역의 정상 구성이 확인됐다
                                  └ ⚠️ **JPYC 이자 배너(「最大年2%」) 2주 연속 미노출 (2026-09-21)** — 자리에 「JPYC: Japan's 1st Approved Digital Yen! / How to Buy & Use JPYC」 구매 가이드 배너가 온다.
                                     비로그인 기준 2회 연속이므로 **이자 배너 소멸 쪽으로 기운다** — 다만 로그인 확인 전까지 단정하지 않는다(§4)
                                  └ 🆕 **2026-09-21 출석 CTA 라벨** — mini·Beta 웹 모두 비로그인에서 **「Get Lucky Ball」**(프로덕션은 **「Check In」**). 같은 버튼의 상태·환경 변형이다(§2-2 `reward_checkin_01`)
...
pay_qr_scan_01 ⚠️잠정              ✅ **QR 스캔 화면 실측 (2026-09-07 프로덕션 로그인 · /pay/qr/mpm)** · 🆕 **한국어 라벨 확정 (2026-09-14)**:
                                  「오프라인 결제 QR 스캔하기 / 매장에 부착된 결제 QR을 스캔해 주세요. 스캔 후 결제가 진행돼요.」 · 권한 팝업 「카메라 접근을 허용해 주세요. / QR코드를 스캔하려면 카메라 접근 권한이 필요해요. 설정에서 카메라 접근을 허용해 주세요.」 **[권한 허용하기]·[취소]**
...
pay_campaign_offline_01 ⚠️잠정     🔴 **오프라인 결제 캐시백 캠페인 (2026-09-14 신설 · `/pay/campaigns/offline` · 비로그인 열람 가능 · 프로덕션·Beta 공통)**
                                  ...
                                  └ 구성: **[View my cashback]** → 🔒 `?campaignStatus=view`(로그인 필수) / 「Event details」 혜택 2블록(10% 캐시백 최대 50 USDT · 누적 500 USDT↑ 시 **500 USDT 추첨 10명**) /
                                     **「Participating merchants」 지역 탭 4종(Gangnam-gu · Seongdong-gu · Yongsan-gu · Mapo-gu)** + 가맹점 카드(상호·카테고리·주소) /
                                     「How to use」 5단계 / 「Compared to overseas cards」 3블록(수수료 0% · 연 3% 이자 · 은행 환율) / FAQ 5종 + [View Unifi FAQ] / 유의사항 9종 / 하단 CTA **[Pay with QR]**
                                  └ 가맹점 카테고리 실측 5종: **F&B · Clothing/Fashion Accessories · Pharmacy/Medicine · Clinic · Services/Other** — 카드 클릭 시 **Google Maps 검색으로 외부 이탈**(Unifi 화면 아님)
                                  └ 🆕 **가맹점 제휴 주체는 「ICB 파트너」**(9/10 공지) · **ZeroPay 가맹점 연동 예정**("will be added soon") — 향후 가맹점 범위 확대 예고
                                  └ 정책(9/10 공지 전문): 기간 **2026-09-10 ~ 10-31 23:59(UTC+9)** · **방한 외국인 대상, 한국 거주 한국인 제외** · USDT QR 결제만 인정 ·
                                     캐시백 결제 후 자동 지급(10~15분 소요 가능) · 1인 누적 상한 **50 USDT** · 추첨 지급 **2026년 11월 중순 예정** · 최대 **550 USDT**
                                  └ ⚠️ **[View my cashback]가 로그인 상태에서 무반응** — 실제 클릭·`?campaignStatus=view` 직접 진입 모두 화면 변화 없음. **캐시백 조회 화면 미실측 · FE 확인 권장**(§4)
                                  └ ⛔ **어휘 미확정 — 부여 금지**(§4 승인 대기 ⑫ · ⑦ `pay_` 주기능 확정과 묶어 결정)
...
- [ ] ⚠️ 잔여 항목 실측 (팝업·조건부: 뽑기 결과, 송금 2단계·QR, 거래 상세, 플러스 모드 상세, 지원 네트워크(브릿지) 상세, `/payout`, 계정 탈퇴, 황금럭키볼, **위임(Delegate) 입력·확인**, **외부 지갑 연결 플로우**, **mini Assets·My 탭**)
  ...
  - 2026-09-14 실측 승격: **오프라인 결제 캐시백 캠페인 화면**(`/pay/campaigns/offline` — 가맹점 목록·정책 전문) · **QR 스캔 화면 한국어 라벨** · **홈 헤더 아이콘 GA 어휘 3종** ·
    **`/setting` 「선호 스테이블 코인」 한국어 라벨** · **`/reward/usdt` 부스트 정책 한국어 전문** · **`/interest/usdt` 빈 상태** · **K-Pick 카테고리 영문 탭 라벨** · **클리닉 필터 영문 라벨**
...
- [ ] 🔴 **프로덕션 리워드 탭 스켈레톤 — 2026-09-21 「로그인 한정」으로 범위 축소 (최우선 유지)**: **비로그인은 정상 렌더로 해소**됐으나 **로그인 상태에서는 20초 후에도 대시 31개**로 고착한다(2회 재현).
  ⚠️ `ACCESS_DENIED`(401)는 **비로그인에서도 찍히는데 화면은 정상**이므로 **401은 원인이 아니다** — #7의 진단(401=고착 원인)을 정정한다.
  ⛔ **스켈레톤은 7/27 이후 환경을 옮겨 다니며 여섯 번 나타났다 뒤집혔다**(프로덕션↔Beta↔mini, 로그인↔비로그인). **종결하지 않고 매 회차 로그인·비로그인 양쪽을 확인**한다. 아래는 #7 시점 기록(이력 보존).
  - (이력 보존) `www.unifi.me/benefits/daily-mission`이 **비로그인(인앱)·로그인(Chrome) 모두 15초 후에도 대시 33개**로 고착한다(3회 재현).
  콘솔 **`ACCESS_DENIED`(401)**, `api-reward.unifi.me/v1/mission-users` **401**(같은 도메인의 `/v1/token-info/country`는 200).
  **Beta 웹·Beta mini 리워드 탭은 정상 렌더**돼 #6(프로덕션 정상 · Beta mini 고착)과 **환경이 정반대로 뒤집혔다**.
  가려진 영역: **출석 체크 진행도 · 게임 미션 6종 · 럭키볼 요약 · 카운트다운** — 이 회차 재확인 불가. **FE 확인 권장 · 다음 회차 최우선**
- [x] ~~🔴 **신규 (2026-09-07) — mini Reward 스켈레톤 재발**~~ → ✅ **2026-09-14 해소**(비로그인 실측 · 대시 0). 남은 스켈레톤은 위 프로덕션 건이다. 상세는 아래 이력 보존.
  - (이력 보존) **mini Reward 스켈레톤 재발 (2026-09-07)**: #5에서 "전 조합 해소·종결"한 직후 **4일 만에 Beta mini `/benefits-mini/daily-mission`에서 재현**됐다.
...
- [ ] **🆕 Beta 홈 「함께하면 더 큰 혜택, KAIA & USDT」 부제 수치 이상** — **"最大0%の報酬" / "Up to 0% rewards"** 노출(프로덕션 4.1%). **2026-09-21까지 8주 연속 재현**(영문 UI에서도 동일), 2026-09-07 **로그인 상태에서도 동일** — 데이터 미로딩 시 0 폴백으로 보이며 **FE 확인 권장**(비로그인 한정이 아님이 확인됐다)
...
- [ ] **mini 이자 노출 정책 충돌 확인** — 위키 스펙은 "mini 예치금·이자 노출 불가"인데 Beta mini에 이자 표기가 **5주 연속 실측**됐다. 2026-09-03 수치가 「최대 연 5%」 → 「最大年2%」로 하향.
  ...
  🔴 **2026-09-21 — 「mini 한정」이 아니다**: **프로덕션 풀 모드 로그인 `/my`의 JPYC 카드에 「최대 연 2%」**, **Beta 로그인 `/my`에 「Max 2%」**가 붙는다.
  즉 2%는 mini 전용 표기가 아니라 **JPYC 이율 자체가 2%**일 가능성이 크다 → **§0 표의 「JPYC 이자 제공 (최대 연 5%)」 기재가 낡았는지 확인 필요**.
  한편 **mini Reward 탭의 이자 배너(「最大年2%」)는 2주 연속 미노출**(자리에 JPYC 구매 가이드 배너) — **배너는 사라지고 카드 배지로 옮겨간 것**으로 보이나 단정하지 않는다
...
- [ ] **K-Pick 탭의 프로덕션 GNB 승격 추적** — Beta·mini는 5탭(K-Pick 포함)인데 프로덕션은 **8주째 4탭**이다(2026-09-21 비로그인 재확인). 승격 시점 확인.
  ⚠️ 🔴 **2026-09-21 상황이 뒤로 갔다** — 프로덕션 **`/benefits`(허브)가 홈으로 리다이렉트**되기 시작했다. `/benefits/k-pick/*`는 열려 있으나 **허브 진입점이 닫혔으므로 「GNB 노출만 남았다」는 기존 판단은 보류**한다
...
- [ ] **⛔ 승인 대기 ⑫ (신규 2026-09-14)**: 🔴 **오프라인 결제 캐시백 캠페인 화면 어휘** — `/pay/campaigns/offline`이 사용자 화면으로 출시됐다(§2-9).
  후보 ⓐ `pay_campaign_offline_01`(+ 캐시백 조회 `pay_campaign_offline_01_01`) ⓑ 프로모션 주기능으로 보고 `promo_pay_offline_01` ⓒ `pay_` 하위 `pay_cashback_01`.
  ⚠️ **캠페인성 화면을 `pay_`(기능)로 볼지 `promo_`(캠페인)로 볼지**가 쟁점이다 — 승인 대기 ⑦(결제 주기능)·프로모션 프리픽스 확정과 **묶어 결정**. **확정 전 부여 금지**
- [ ] 🆕 **mini 홈 ↔ K-Pick 탭 중복 판단** — mini 홈(`/benefits-mini`)이 K-Pick 화면과 사실상 같아졌다(§2-7).
  ~~🔴 2026-09-07 중복이 완전해졌다~~ → 🔴 **2026-09-14 판정 번복**: mini 홈에 **바우처 12종·큐레이션 3건·K-뷰티 상품 카탈로그가 복귀**했고 Beta Web `/benefits` 허브에는 카탈로그가 없다.
  즉 **상단 허브 영역은 동일하고 mini 홈에만 카탈로그가 덧붙는 구조**다. 2주 만에 구성이 뒤집혔으므로 **구성 스냅샷이 아니라 「허브 + (환경별) 카탈로그」 구조로 어휘를 정하는 편이 안전하다** — 결정 필요
  → 🔴 **2026-09-21 재번복**: Beta 웹 `/benefits`가 **풀 카탈로그로 전면 개편**되면서 mini 홈과 **다시 완전히 같아졌다**(문구·상품·클리닉 랭킹까지 일치).
  **#6 완전 중복 → #7 mini만 카탈로그 → #8 다시 완전 중복**으로 3주 연속 뒤집혔다. **구성 스냅샷 기반 어휘는 쓰지 않는다**는 결론이 확정적이다 — 「허브 + (환경별) 카탈로그」 구조로 정의하는 안을 **사용자 결정 대상으로 올린다**
...
- [x] ~~🔴 **Beta·mini 로그인 영역 점검 불가 (#7 이월 3번)**~~ → ✅ **2026-09-21 해소 — 점검 창구가 확인됐다.**
  **Chrome은 Beta 도메인(`unifi-web.line-apps-beta.com`) 접근이 차단**되지만(#7과 동일), **인앱 브라우저는 Beta에 접속되고 사용자 로그인 세션이 그대로 살아 있다**.
  ➡️ **앞으로 Beta·mini 로그인 점검은 인앱 브라우저로 한다**(프로덕션 로그인은 Chrome·인앱 양쪽 가능).
  🆕 **Beta `/my` 로그인 첫 실측 (2026-09-21)**: 액션 4종 **Send·Deposit·Swap·Bank Withdraw** · USDT 「Max 7%」 · KAIA 🔴 **「Max additional 3%」 + "Stake KAIA and Earn up to 0% annual interest"** ·
  JPYC 「Max 2%」 · **비상장 토큰 4종 BO·GRT·SIK·YOO(각 10,000 · 「Unlisted」 배지)** 재확인 · Owned NFTs 0 · GNB **5탭**
...
- [x] ~~🆕 **Apps 카테고리 `SocialFi` 소멸 확인 (2026-09-07)**~~ → ✅ **2026-09-14 2주 연속 7종 확인으로 카테고리 폐지 확정**(AI·CONTENT·DePIN·GAME·Payment·SOCIAL·ETC · 26 Apps 유지). 추적 종결
- [ ] 🆕 **신규 (2026-09-14) — Apps 「나의 관심 Apps」 섹션의 정체 확인** — 게임 프로모션 캐러셀 아래에 신설된 섹션([전체보기] 동반)이 즐겨찾기인지 추천인지, 별도 라우트가 있는지 미확인(비어 있는 상태로만 관측)
- [x] ~~🆕 **신규 (2026-09-14) — 캐시백 조회 「View my cashback」 무반응**~~ → ✅ **2026-09-21 해소 — 정상 동작한다.** 동작하지 않는 진입점이 아니었다.
  **별도 화면이 아니라 같은 페이지에 「My Cashback Status」 블록이 인라인 삽입**되는 방식이다(「How to use」와 「Compared to overseas cards」 사이 · 진입 후 URL에서 쿼리가 제거된다).
  구성: **Cashback you've received 0 USDT** · **Amount paid offline 0 USDT** · **History** 링크 · 추첨 응모 상태 **「Not entered」**("Spend 500 USDT for automatic entry.").
  ⛔ **Screen ID 영향 — 승인 대기 ⑫ 재검토**: 별도 화면이 아니라 **같은 화면의 로그인 상태 변형**이므로 `pay_campaign_offline_01_01`(별도 ID) 안은 근거가 약해졌다
- [ ] 🔴 **신규 (2026-09-14) — mini 「외부 게임 미션」 한국어 노출 재발** — #5에서 "섹션 소멸과 함께 종결"로 닫았던 XLT 누락 의심 건이 **Game mission 섹션 부활과 함께 재현**됐다(영문 UI인데 항목명만 한국어). XLT 키 확인 대상
- [ ] 🆕 **신규 (2026-09-14) — K-Culture 상품 카드 태그의 언어 혼재** — 같은 카드에 「ソウル」·「仁川」(ja)과 「Show Ticket」·「KPOP」·「Aquarium」(en)이 섞여 노출된다. 태그 원본이 다국어화 대상인지 기획·XLT 확인 필요
- [ ] 🆕 **신규 (2026-09-14) — mini 홈 「내 예약」 툴팁의 노출 조건** — #6은 로그인 한정으로 기재했으나 이번 회차 **비로그인에서도 노출**됐다. 조건 재확인 필요
- [ ] 🔴 **KAIA 스테이킹 이율의 문서 파급** — ~~2026-09-14: 3곳 모두 4.1%~~ → 🔴 **2026-09-21 정정: 언어별로 갈린다(ko 4.2% / en 4.1% / Beta 0%)**.
  **어느 값이 정본인지부터 확정해야** 위키·XLT·프로모션의 교체 방향을 정할 수 있다 — 아래 「이율 표기 3중 불일치」 항목과 같은 건이다(IA 점검 범위 밖, 별도 작업)
- [ ] 🔴 **신규 (2026-09-21) — K-Pick 가격 표기 이상 (FE·데이터 확인 권장)**: 같은 상품·같은 할인율인데 **허브(`/benefits`)와 카테고리 목록(`/benefits/k-pick/*`)의 가격이 다르다**
  (Juno Hair 홍대 ¥26,086↔¥18,941 · Hoso 도산청담 ¥3,030↔¥1,265 · Marzia 힐링스파 ¥35,435↔¥16,106).
  특히 **마리엠 헤어&헤드스파가 허브에서 ¥25,294,160**(카테고리 ¥21,821)으로 나오는 것은 명백한 이상치다. **Beta 웹·mini 양쪽에서 동일 재현**되므로 환경 문제가 아니라 데이터·바인딩 문제로 보인다
- [ ] 🆕 **신규 (2026-09-21) — 프로덕션 `/benefits` 홈 리다이렉트의 의도 확인**: #7까지 K-Pick 허브(배너 3종)였던 주소가 홈으로 리다이렉트된다.
  하위 `k-pick/*`·`daily-mission`은 그대로 열려 있어 **허브 진입점만 닫힌 상태**다. K-Pick GNB 승격 추적과 **반대 방향**이므로 의도된 정리인지 확인 필요
- [ ] 🆕 **신규 (2026-09-21) — `/apps/market` Ranking 탭의 정렬 기준·기간 옵션 확인**: 비로그인에서는 순위 값이 전부 `-`로 비어 있어 기준(거래량/거래액)과 「Daily」 외 기간 옵션을 확인하지 못했다
- [ ] 🆕 **신규 (2026-09-21) — K-Pick 허브 카테고리 아이콘 ↔ 라우트 불일치**: 허브 아이콘이 **5종**(Clinics·K-Beauty·Esthetic·Tours & Tickets·Activities)인데 카테고리 라우트는 **3종**(`beauty|shopping|pop`)뿐이다.
  Screen ID 어휘를 아이콘 기준으로 잡을지 라우트 기준으로 잡을지 **승인 대기 ③·⑩과 묶어 결정** 필요
- [x] ~~🆕 **신규 (2026-09-14) — Apps 「나의 관심 Apps」 섹션의 정체 확인**~~ → ✅ **2026-09-21 해소 — 로그인 전용 섹션**이다. 같은 날 비로그인에서는 미노출, 로그인에서는 캐러셀 아래에 노출됨을 대조 확인했다.
  Editor's Pick을 **대체하지 않고 추가**되며, 실측 계정에서는 **빈 상태**였다(즐겨찾기/추천 여부는 등록된 계정 필요 — 남은 확인은 이 한 가지)
- [ ] 🔴 **신규 (2026-09-21 · 최우선) — KAIA 스테이킹 이율 표기가 환경·언어마다 갈린다 (XLT 확인 대상)**: 같은 날 실측인데 **세 값이 다르다**.

  | 화면 | 값 |
  |---|---|
  | 프로덕션 **한국어**(로그인) — 홈 배너·내 자산 KAIA 카드·`/reward/kaia` **3곳 모두** | **최대 연 4.2%** |
  | 프로덕션 **영문**(비로그인) — 홈 배너·`/reward/kaia` | **up to 4.1%** |
  | **Beta 영문**(로그인) — 내 자산 KAIA 카드 | **up to 0%** |

  ⛔ **#7의 「4.2% → 4.1% 하향, 3곳 일치」 판정을 정정한다** — #7이 4.1%로 본 3곳은 한국어 화면이었는데 지금 한국어는 **4.2%**다.
  한국어 XLT가 되돌아간 것인지 #7 기록 오류인지는 미확정이나, **현 상태가 ko 4.2% / en 4.1%로 갈려 있다는 사실은 확정**이다 — **언어별 XLT 값 불일치로 확인 필요**.
  또한 Beta의 **「0%」가 홈 배너뿐 아니라 내 자산 KAIA 카드에도 나타난다**(§4 「Beta 0%」 항목과 같은 뿌리로 보인다)
- [ ] 🔴 **신규 (2026-09-21) — mini GNB가 3탭이다**: Beta mini(`/benefits-mini`)의 하단 탭이 **Home · Reward · My 3종**으로만 렌더된다(**K-Pick·Assets 없음**).
  IA §0 표·§2-7의 **「mini Beta = 5탭」 기재(2026-07-30·08-03 근거)와 어긋난다**. 같은 시점 **Beta 웹은 5탭**(Home·K-Pick·Reward·Assets·My)으로 정상이므로 **mini 한정 축소**다.
  mini 홈 자체가 K-Pick 카탈로그가 된 것과 맞물린 정리로 보이나, **다음 회차 재확인 후 §0 표를 고친다**(이번 회차에는 실측 사실만 기록)
- [ ] 🔴 **신규 (2026-09-21) — 프로덕션 풀 모드 내 자산 JPYC 카드에 「최대 연 2%」가 노출된다**: 종전 충돌 기록은 **mini 한정**이었는데(§0 JPYC 이자 행 「제공 (최대 연 5%)」),
  프로덕션 로그인 `/my`의 JPYC 카드에 **「최대 연 2%」** 배지가 붙는다(Beta도 「Max 2%」 동일). **§0 표의 「최대 연 5%」 기재가 낡았을 가능성** — 정책 확인 필요
