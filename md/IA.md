...
- [ ] ⚠️ 잔여 항목 실측 (팝업·조건부: 뽑기 결과, 송금 2단계·QR, 거래 상세, 플러스 모드 상세, 지원 네트워크(브릿지) 상세, `/payout`, 계정 탈퇴, 황금럭키볼, **위임(Delegate) 입력·확인**, **외부 지갑 연결 플로우**, **mini Assets·My 탭**)
  ...
  - ⚠️ **4주 연속 미완 — 서비스측 이슈로 보고(범위 축소됨)**: 리워드 탭(`/benefits/daily-mission`) 요약 카드·게임 미션이 **스켈레톤에서 진행되지 않음**. 프로덕션(7/27 로그인)·Beta(7/30·8/3 로그인, **8/10 비로그인**) 재현.
    2026-08-03 네트워크 실측: 정적 자산·API 모두 **정상 200**(`POST /unifi/v1/mission-users` 200 포함)이고 콘솔 에러도 없는데 **UI만 로딩 스켈레톤에 고착** → 프런트 렌더 이슈로 추정.
    ※ **2026-08-10 신규 사실**: 같은 증상이 **Beta 비로그인에서도 재현**됐고, **프로덕션 비로그인은 정상 렌더**다 → "로그인 분기 문제"가 아니라 **Beta 환경(빌드) 문제**일 가능성이 높다. 프로덕션 로그인에서 재현되는지가 남은 분기점(로그인 세션 필요)
- [ ] **로그인 세션 전무 — 사용자 로그인 필요(악화)**: 2026-08-10 기준 프로덕션은 물론 **Beta 로그인 세션까지 만료**(`/my` → `/auth/sign-in`, Google에 "최근 로그인 했어요" 배지만 남음).
  미점검으로 남음 — `/my`(내 자산)·`/setting`(마이)·`/notification`·`/my/token/transaction`·`/apps/my-page/nfts`·`/transfer`·`/deposit`·`/apps/trade/swap`, **외부 지갑 연결 메뉴 재확인**, **mini 로그인 상태(잔액 배너·내 예약 타일)**
