...
| **mini × KR IP × 로그인** | ✅ 실측 (Beta mini 2026-07-30·08-03·08-10) | Chrome (Beta mini) |
...
- [ ] 🆕 **Beta 디버그 패널 활용 여부 결정 (2026-08-10 발견)** — Beta 화면 좌측 톱니에서 **「국가 모킹」·「알림 테스트」·「결제 가맹점 진입」** 버튼 확인.
  - **「국가 모킹」은 미실측 축(JP IP·Wallet Mode 강제 국가)을 실측으로 승격할 수 있는 유일한 수단**일 수 있다. 다만 클라이언트 상태를 바꾸는 개발 도구라 **사용자 승인 없이는 쓰지 않는다** — 다음 회차에 승인되면 JP·US IP 화면을 실측한다
  - **「결제 가맹점 진입」은 Unifi Pay의 사용자측 결제 화면이 Beta에 준비 중이라는 신호**다. 결제 플로우라 이번엔 미진입 — 진입 승인 시 `pay_` 어휘 결정의 근거가 된다
