...
`(New)` 전수 추출·단일 프레임 모드와 **별개의 세 번째 모드**다. 디자이너가 번역 대상 문구 근처에 `xlt:`로 시작하는 Figma 코멘트를 달면, 그 코멘트가 가리키는 텍스트만 골라 번역한다. (기존 두 모드는 그대로 유지.)

**트리거**: 사용자가 "Figma 코멘트(`xlt:`)로 표시한 문구만 번역" 류로 요청 + Figma URL(페이지 또는 프레임) 제공 시.

**대상 선택자는 위키 번호가 아니라 `xlt:` 접두사다.** 위키 Screen 표 Description의 번호(`1.`, `2.`…)는 미해결 코멘트의 y정렬 순번이라 코멘트 추가/해결 시 밀린다. 이 모드는 **실시간 Figma 코멘트를 다시 읽어 `xlt:` 접두사로 대상을 선택**한다 — 위키 번호는 사람이 교차 확인하는 라벨로만 쓴다.

**절차:**

1. `setCurrentPageAsync`로 대상 페이지 설정(또는 프레임 URL이면 그 프레임으로 한정).
2. **`xlt:` 코멘트 수집** — `md/wiki.md`의 코멘트 조회 REST 절차(`GET /v1/files/{fileKey}/comments`)를 재사용한다. 미해결(`resolved_at` null) **루트** 코멘트 중 `message`가 `xlt:`로 시작하는 것만 필터(대소문자 무시·앞 공백 trim)하고, 각 `{id, node_id, node_offset{x,y}, message}`를 보관한다.
3. 각 `xlt:` 코멘트의 **대상 텍스트를 매칭**한다(아래 매칭 알고리즘).
4. 매칭된 텍스트를 중복 제거한다 — **동일 한국어 원문 = 1개 키** 불변 규칙(Step 2) 준수.
...
2. **아니면(프레임/컨테이너)** → 코멘트 절대점을 **포함하는** 텍스트 bbox를 우선 선택, 없으면 중심거리 **최근접**(합리적 반경 내) 텍스트를 선택한다 (`매칭방식=좌표`).
3. 반경 내 후보가 없으면 → **`매칭 실패`로 표시**하고 자동 선택하지 않는다.

```python
# text_nodes: [{ "t": 원문, "abs": [x, y, w, h] }]  (use_figma findAll + absoluteBoundingBox)
...
| 위키번호(참고) | xlt: 메시지 | 프레임 | 매칭된 텍스트 | 매칭방식 |
|---|---|---|---|---|
| 2 | xlt: 이 버튼 문구 | (New) 자산 전송 팝업 | 전송하기 | 직접 |
| 5 | xlt: 안내 문구 | (New) 자산 전송 팝업 | (매칭 실패) | - |
