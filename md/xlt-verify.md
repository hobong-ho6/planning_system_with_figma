...
**서비스 간 공통 키의 45%가 값이 갈린다.** `Unifi`(2,131키) ∩ `Dapp Portal`(1,596키) = **공통 336키**, 그중 **152키(45%)** 가 다르다. 두 서비스가 **서로의 키를 구값 사본으로 보유**하고 있다 — `Dapp Portal`은 `UF_`(UIT) 구값 88키를, `Unifi`는 `unifi_promotion_` 구값 44키를 들고 있다.
