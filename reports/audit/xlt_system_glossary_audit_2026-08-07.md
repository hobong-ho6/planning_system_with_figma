...
| `Kaia Wallet` | `usdt_send_guide_title` | `전송**시** 주의사항` |
| `Kaia Wallet` | `web_send_passcode_secret_logout` | `5회 이상 입력 실패**시** 자동 로그아웃 됩니다` |
...
| **P0 `언어 혼입` — 언어 선택 라벨** | **18**(5키) | `UF_settings_display_language_ko/ja/th/cn`·`_currency_thb`는 **5개 언어 전부 자기 언어 표기**(`한국어`·`日本語`·`ภาษาไทย`·`中文(繁體)`·`THB (฿)`)로 **의도된 값**이다. 언어 선택기는 어느 로케일에서든 각 언어를 그 언어로 보여준다. 한 키가 여러 칸에서 걸려 이슈 수(18)가 키 수(5)보다 많다 |
| **P0 `언어 혼입` — zh_TW `・`** | 10 | 구분자 `・`(U+30FB)가 검증기의 가나 범위에 걸린다. 중국어에서 정상 사용 → **검증기 한계**, §6-2 |
| | **합계 4,975** | 그중 §3·§4-2로 승격한 건 제외 |
...
### 7-1. 서비스 간 값 분기 — 공통 335키 중 151키(45%)

`Unifi`(2,130키) ∩ `Dapp Portal`(1,596키) = 335키, 그중 **151키가 값이 다르다**. 두 서비스가 **서로의 키를 구값 사본으로 보유**한다.

```
갈리는 키 프리픽스:  UF_ 87 · unifi_ 44 · app_ 7 · ms_ 6 · mini_ 4 · Minidapp_ 1
```

> ⚠️ **같은 리포트 안에서도 조회 시점에 따라 수치가 달라진다.** §7-3의 키 삭제 **전** 조회로는 공통 **336키·상이 152키·`UF_` 88**이었다 — 삭제된 `UF_floating_jpyc_banner_title`이 값이 갈리는 공통 키였기 때문이다. 위 수치는 **삭제 후 스냅샷**이며 서비스 표의 2,130키와 정합한다.
