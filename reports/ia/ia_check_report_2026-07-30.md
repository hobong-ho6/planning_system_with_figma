...
| 버전 | 내용 |
|---|---|
| `v8` | IA 점검 리포트 탭에 #2 회차 패널 추가 |
| **`v9`** | **「전체 IA 구조」 인터랙티브 표 신설** — 1~3뎁스 화면 계층 + 환경별(Web·LIFF·Wallet Mode·mini) 제공 여부를 한 화면에서 확인. 환경 필터 · 미제공 화면 숨기기 · 1뎁스 접기/펼치기 동작 |

데이터는 `index.html`의 **`IA_DATA` 배열** 한 곳에 있고 표·필터·접기는 자동 렌더됩니다. **`md/IA.md`가 바뀌면 같은 작업에서 이 배열도 갱신**하는 규칙을 IA.md 머리말과 스케줄 태스크 절차(가이드 갱신 2단계)에 신설했습니다 — 두 곳이 어긋나면 팀원이 보는 IA가 틀리기 때문입니다.
