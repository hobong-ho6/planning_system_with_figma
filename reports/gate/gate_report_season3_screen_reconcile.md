...
# 부록 7 — OA Alt 메시지(altText) 다국어 번역 (v37)

사용자가 OA 번역표에 `Alt` 행(`미션 당첨금이 지급되었습니다.`, 4개 언어 공란)을 추가한 뒤 번역 요청.

## 번역

| 언어 | 값 | 길이 |
|---|---|---|
| ko_KR | `미션 당첨금이 지급되었습니다.` | 16 |
| ja_JP | `ミッション当選金が支給されました。` | 17 |
| en_US | `Your mission prize has been paid.` | 33 |
| th_TH | `จ่ายเงินรางวัลภารกิจเรียบร้อยแล้ว` | 33 |
| zh_TW | `任務獎金已發放。` | 8 |

용어집(v4.0) 준수 — `당첨금` en `prize` · ja `当選金` · th `เงินรางวัล` · zh `獎金` / `미션` en `mission` · ja `ミッション` · th `ภารกิจ` · zh `任務`.

**동일 화면 내 표현 일관성**(같은 OA의 No 1·2와 대조):
- ja `支給されました` — No 2 `…に支給されました。`와 동일 어형
- en `prize has been paid` — No 2 `Your {{amount}} JPYC prize has been paid to …`와 동일
- th `จ่ายเงินรางวัลภารกิจ…เรียบร้อยแล้ว` — No 1 `จ่ายเงินรางวัลภารกิจ {{amount}} JPYC เรียบร้อยแล้ว!`와 동일 구문
- zh `已發放` — No 2 `已發放至 …`와 동일. 마침표는 ko·ja·zh만(합쇼체·평서문), th는 관습상 미사용

## altText는 bubble JSON에 넣지 않는다 (중요)

LINE Flex의 `altText`는 **메시지 봉투**에 속하는 값이다.

```json
{ "type": "flex", "altText": "미션 당첨금이 지급되었습니다.", "contents": { …bubble… } }
```

`md/OA.md`의 산출물 규격은 **bubble JSON**(스펙과 동일 레벨)이고, **Flex Message Simulator도 bubble(contents)만 입력으로 받는다**. 따라서 ⓐ bubble에 `altText` 키를 넣으면 잘못된 속성이 되고 ⓑ 봉투로 감싸면 사용자가 방금 통과시킨 시뮬레이터 검증이 깨진다. → **`flex_OA_Reward_Confirm_*.json` 5파일과 zip은 수정하지 않았다**(zip v2 유지). 발송 시 OA 콘솔·Messaging API의 `altText` 필드에 위 표의 언어별 값을 넣는다.

## 검증

기존 OA 5행 + Alt 1행 = **6행 전수** 재검증.

```
P0 0건 | P1 15건 | P2 2건
```

Alt 행 P1 3건 = `당첨` → en `win` / th `ถูกรางวัล` / zh `中獎` — **전건 오탐**. 용어집에 `당첨`(win)과 `당첨금`(prize)이 각각 등재돼 있고 이 행은 `당첨금`이므로 `prize`·`เงินรางวัล`·`獎金`이 정본이다. 검증기가 긴 용어(`당첨금`)를 우선 매칭하지 못해 `당첨`으로 걸린 구조적 오탐(`md/check.md` 함정 2-1 유형).

기계 보조: 빈칸·nbsp·이중공백·변수 혼입·금지어 **0건** / altText 길이 최대 33자(LINE 상한 400) 여유.

위키 반영 후 6행 모두 **5/5 채움** 확인 · `check_wiki_storage.py` pre/post exit 0.

## (d-2) 🆕 21번 — altText에 금액을 넣을지

현재 Alt는 금액이 없어 푸시 알림에서 `미션 당첨금이 지급되었습니다.`만 보인다. 기존 OA 4건의 altText 관례와 맞추거나, 열기 전 금액을 노출하려면 `미션 당첨금 {{amount}} JPYC가 지급되었습니다.`처럼 `{{amount}}`를 넣는 안도 가능하다(변수는 봉투 텍스트에서도 치환 가능). 사용자 결정 사안.
