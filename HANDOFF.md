...
- [x] ~~**P0**: 용어집 CMS 반영 · 가이드 zip 갱신·게시~~ — **완료**(v4.1→v4.6 누적 반영 · 가이드 v14→v21 게시). 라이브 버전은 「프로젝트 정보」 참조.
- [x] ~~**P1**: 시즌3 「꼭 확인해 주세요」 XLT 키 할당~~ — **완료**(2026-08-06 세션 #11). 유의사항 8건에 **재사용 6키 + 신규 5키** 부여, 번역표 57키.
- [x] ~~**P1**: UIT 치환자 표기 확인~~ — **원인 규명 완료**(2026-08-07 세션 #17). **문서 규칙(`{{0}}`)이 맞았다.** `UF_` 정본은 **`Unifi` 서비스**이고 거기서 `{{0}}`가 279키다. 세션 #16이 대조한 `Dapp Portal`은 잘못된 서비스였고 그 안의 `UF_` 32키가 `{0}` 사본이었다. #16의 가설 ⓐⓑⓒ 중 어느 것도 아니고 **조회 대상이 틀렸던 것**. → 아래 「Dapp Portal 구값 사본」 항목으로 승계.
...
- [x] ~~**P1**: 변경 5키 XLT 시스템 등록값 갱신~~ — **해소 확인**(2026-08-07 세션 #17). `compare_wiki_xlt.py`로 시즌3 위키를 대조하니 ⓔ 분기 10건이 **전부 `Dapp Portal`(정본)과 일치**했다. 위키가 앞서간 상태가 아니라 **시스템 갱신까지 끝난 상태**였다. 남은 것은 `Unifi`의 구값 사본(위 항목).
...
- [x] ~~**P1**: XLT 등록값 API 연동 구현 여부 결정~~ — **구현 완료**(2026-08-07 세션 #17, `847e2de`). 스크립트 3종 + `md/xlt-verify.md`. 다만 검토 보고서가 권고한 **`validate_translation.py --registry` 옵션은 만들지 않았다** — 레지스트리를 검증기에 주석으로 붙이는 대신 **레지스트리 자체를 검증 대상으로 넣는 방식**(`verify_xlt_glossary.py`)이 더 단순하고, 등급·기준선을 건드리지 않아 「P1 강등 기각」 권고도 자동으로 지켜진다. `test_validation.py`는 전체 통과(기존 로직 무변경).
...
- ~~**`unifi_promotion_jpyc_info_oa_desc`는 ⓐ안이 정본**(2026-08-05) — `이후 연계해 참여할 수 있는…`~~ → **2026-08-06 번복**. 사용자가 **문구를 한 번 더 검토해 갱신**했고 위키 Screen 표를 직접 수정했다: **`다양한 이벤트 소식을 가장 빠르게 받아볼 수 있는 Unifi의 LINE 공식 계정 친구를 유지해주세요.`** 5개 언어 재번역 완료(위키 v76 · 엑셀 ALL v19·LV v10). **이 값이 현재 정본**이며 ⓐ안으로 되돌리자는 제안 금지.
...
- ~~**OA `총 2개의 럭키볼`은 고정 문구**(2026-08-05)~~ → **2026-08-06 번복**. 사용자가 **문구를 한 번 더 검토해 갱신**했다: 화면 키 `bottomsheet_signup_text3`와 OA 2화면 모두 **`…친구에게도 JPYC 선물하면 JPYC 럭키볼 최대 20개를 더 받을 수 있어요.`** (「친구가 가입하고 OA 친구 추가하면 총 2개」 → **「선물하면 최대 20개」**로 조건·수치가 함께 바뀜). 5개 언어 재번역 + **OA Flex JSON 10개·zip 2종 재생성** 완료. **변수화하지 않는다는 원칙은 유지**(수치는 고정 문구).
...
| 2026-07-24 | **History 같은 날 1행 병합** · **팝업 겹침 화면은 어노테이션 육안 검증 필수**(좌표 매칭이 z-order 미인식) · **정책 추출은 순수 `xlt` root만 제외** | 3건 모두 `CLAUDE.md`·`md/wiki.md`에 규칙으로 반영 완료 |
