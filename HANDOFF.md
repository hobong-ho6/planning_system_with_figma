...
| [glossary-guide](handoff/projects/glossary-guide.md) | `hogeun` | 용어집 + 기획자 가이드 **(전역 자원)** | 09-14 | 용어집 **v5.5** · 가이드 **v40 게시 완료** · 태그 **`guide-v40`** — 「고유명사 공식 표기 확인」 규칙 반영 · changelog v5.4·v5.5 상세 백필 · **미결 1건**(`UF_clinic_detail_review_source` ja 검토) |
...
| [unifi-mini-v2](handoff/projects/unifi-mini-v2.md) | `hogeun` | 위키 `4704515582` + 하위 **LPC `4727978725`** · **25화면** | 09-14 | 본문 **v185** · LPC **v50** — ⛔ **혜택 비율 확정: 할인 5% · 캐시백 10% · 합산 소구 15%** · **§9-7 연동 필드 규칙 신설**(6필드·80셀, 비율 변경 시 동반 갱신) · **`voucher_product`(UIT) 신설·등록**(`my_voucher_detail`만 · LV·UIT × beta·prod 4곳 동치) · ⛔ `k_pick_shopping_product`·리뷰 본문 **관리 대상 아님** · ⛔ **미결 4건 해소**(`최근 본 상품` `UF_` 유지 · `최대혜택가` 3키 유지 · 판단 3건 승인 · `my_voucher_detail` **병존 유지 확정**) · 🔴 **한정해제 8개 파라미터** 미결 → 파일 참조 |
