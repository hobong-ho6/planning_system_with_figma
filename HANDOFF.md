...
| [cu-qr-voucher](handoff/projects/cu-qr-voucher.md) | `hogeun` | 위키 `4774235795` CU 상품권 전용 동선 · **3화면** | 09-21 | 위키 **v4** · XLT **7키**(5키 신규) · GA 11건 · 첨부 4건 정리 완료 · 게이트 P0 0 — 🔴 **P0 결제 바텀시트 키 중복**: 같은 프레임 `75648:8250`을 다른 세션이 **UIT `UF_voucher_pay_more_cu_*`**로 `4704515582`에 먼저 넣었는데, 이 페이지의 **LV `mini_voucher_cu_payment_done_benefit_*` 4키가 시스템에 등록돼 버렸다**(문구 동일) — 어느 키를 살릴지 결정 전까지 양쪽 수정 금지 |
...
- **⛔ 신규 XLT 키 중복 검사는 레지스트리만으로 부족하다**(2026-09-21 실측) — 같은 날 다른 세션이 **위키·게이트 리포트에만 올리고 아직 등록 안 한 키**는 레지스트리에 없어 「중복 없음」으로 통과한다. 같은 4문구가 `mini_`(등록됨)·`UF_`(미등록) 두 벌로 갈렸다(`handoff/projects/cu-qr-voucher.md` P0). 신규 키 확정 전 **`reports/gate/` 당일 리포트 + 같은 화면을 담은 형제 위키를 ko 문구로 대조**한다
