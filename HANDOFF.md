...
> 마지막 갱신: 2026-08-07 (**세션 #16**) · 작업 PC **`AD03230205ui-iMac.local`** · branch `main` · **핸드오프 본문 커밋 `<이 줄 다음 커밋>`** — 그 뒤에 이 해시를 적는 **후속 커밋 1개가 HEAD**다(정상, 불일치 아님). 세션 작업분 `07b0d72`(검토 보고서 1건)

**진행 중 작업(WIP)**: **없음.** 작업 트리 clean.

**⚠️ 세션 번호가 커밋 로그로만 추정된다.** 기록된 최신은 #13인데 그 뒤 **08-06 저녁 5커밋**(`e338cbf`~`870fc33`)과 **08-07 오전 2커밋**(`f2993ca`·`7077ff3`)이 세션 기록 없이 들어와 있다. 각각 **#14·#15로 추정**해 아래 세션 기록에 커밋 로그 기준으로 요약했다. **#9·#11·#12에 이어 네 번째·다섯 번째 미갱신**이다 — 이 저장소의 반복 실패 지점이다.

**이번 세션(#16) — XLT 시스템 API 타당성 검토 (코드 미작성, 보고서 1건):**

사용자가 크롬 확장 `~/Documents/dec/`(사내 XLT 미리보기 툴)을 주며 "XLT 시스템에서 키를 어떻게 가져오는지 확인하고, 용어집 비교에 직접 쓸 수 있는지 검토"를 요청했다.

- **XLT 시스템에 무인증 읽기 REST API가 있다** — 확장 역공학 + **실호출로 검증 완료**
  ```
  ...
  ```
  `service`=`Dapp Portal` · `device`=`WEB BROWSER` — **export 파일명이 그대로 경로 파라미터**다. 인증 헤더·쿠키 **없음**(확장도 `fetch(url)`을 2번째 인자 없이 호출). 언어당 요청 1건, flat `{키: 번역문}`.
- **실증**: 1,596키 × 5개 언어, 키셋 동일·빈 값 0. `HANDOFF.md`의 "2026-08-06 export 1,596키"와 일치하고, **P1 「구 표기 17키」의 수작업 실측치를 전건 재현**(zh `關注` 8·`追蹤` 3 · ja `友達` 4 · ko `월렛` 13).
- **이 PC에 export 파일이 없는 상태에서 API로는 즉시 조달했다** — `gate_report_nonrealtime_improve_2keys.md:10`에 기록된 "export가 이 PC에 없어 위키 등록값 기준으로 진행" 사고의 실연이자 해결책.
- 보고서: **`reports/research/xlt_api_feasibility_2026-08-07.md`** (327줄, `reports/research/` 신설). 사용자 결정으로 **코드는 만들지 않았다**.

**⚠️ 부수 발견 — UIT 치환자 표기가 시스템 실측과 어긋난다(FE 확인 필요).** 시즌3 57키를 레지스트리와 대조하니 **값 상이 3키가 전부 치환자 표기만** 달랐다 — 위키·엑셀 `{{0}}` ↔ 시스템 `{0}`, 3키 × 5언어 = **15셀**. 세 키 모두 `UF_` = UIT인데 `CLAUDE.md` 규칙은 UIT = `{{0}}`이다. **API 언이스케이프가 아니다** — 1,596키 전수 스캔에서 `{{` 포함은 **정확히 1키**(`UF_max_cashback`)이고 단일 `{n}`은 174키(그중 `UF_` **32키**)로 **두 표기가 공존**한다. `md/guide.md:228`("임의 변경 금지 — 보고만")에 따라 **아무것도 바꾸지 않았다**.
**용어집·가이드는 P0가 전건 해소된 상태다.** 용어집 라이브 **v4.5**(115 terms · `deprecated_terms` 13종), 가이드 라이브 **v19**. 둘 다 2026-08-06 사용자 반영·게시 완료.
...
- [ ] **P1**: **UIT 치환자 표기 확인(FE·UIT)** — `CLAUDE.md` 규칙은 UIT = **`{{0}}`** 인데 **XLT 시스템 실측은 `UF_` 32키가 `{0}`**, `{{0}}`는 `UF_max_cashback` **1키뿐**이다(1,596키 전수 스캔). 시즌3 위키·엑셀과 시스템이 **3키 × 5언어 = 15셀** 어긋난다(`UF_home_jpyc_banner_jackpot`·`UF_floating_jpyc_banner_title`·`UF_floating_jpyc_banner_desc` — 문구는 동일, 치환자만 차이). ⓐ 규칙이 맞고 시스템 32키가 틀림 ⓑ `{{0}}`는 FE 코드 참조 표기일 뿐 저장 표기가 아님 ⓒ 업로드 시 시스템이 정규화 — 셋 중 무엇인지 **FE 확인 필요**. 근거: `reports/research/xlt_api_feasibility_2026-08-07.md` §6. **확정 전까지 아무것도 바꾸지 않는다**(`md/guide.md:228`).
...
- [ ] **P1**: **변경 5키 XLT 시스템 등록값 갱신** — 2026-08-06 위키·엑셀은 새 문구로 반영했으나 **시스템은 아직 구 문구**다(`jpyc_btn_invite`·`bottomsheet_signup_text3`·`jpyc_info_oa_desc`·`terms_valid_entry`·`invite_banner_desc`). 시스템 갱신 전에는 **FE 실제 노출이 바뀌지 않는다**.
- [ ] **P1**: **`mini_luckyball_invite_banner_desc` 공유 키 등록값 결정** — 시즌3는 `최소 100 JPYC`, 캠페인 `4479306980`·XLT 시스템은 `최대 50,000 JPYC`로 **갈려 있다**(2026-08-06 사용자 결정: 시즌3만 반영). 「최대 상한」 ↔ 「최소 보장」으로 의미가 반대이고 **시스템에는 하나만 등록**되므로 어느 쪽으로 등록할지 결정이 필요하다.

### Claude 실행 대기 (승인 시 진행)
- [ ] **P1**: **XLT 등록값 API 연동 구현 여부 결정** — 타당성은 **검증 완료**(`reports/research/xlt_api_feasibility_2026-08-07.md`). 구현 시 순서: ① `scripts/fetch_xlt_registry.py` + `.gitignore` → ② **사용자 export와 전량 대조해 차이 0 확인**(⚠️ 이 검토 PC에 export가 없어 **미수행**. API가 export를 대체한다는 최종 증거이므로 **여기서 실패하면 중단**) → ③ `validate_translation.py` **`--registry` 옵셔널 주석 전용**(등급·건수·종료코드 불변 — **「등록값 유래 P1 강등」은 기각 권고**, 근거 §5-2) → ④ `test_validation.py`에 「registry 지정 전/후 P0·P1·P2 건수 동일」 단언 → ⑤ 문서 갱신(`HANDOFF.md:28`·`CLAUDE.md`·`.gitignore`·`md/check.md`). **정본 조달 정책은 「API 1차 · export 백업」으로 사용자 확정**.
...
### 2026-08-07 — 세션 #16: XLT 시스템 API 타당성 검토 (**코드 미작성 — 보고서 1건**)

- **완료(1커밋 `07b0d72`)**: `reports/research/xlt_api_feasibility_2026-08-07.md`(327줄, `reports/research/` 신설)
  - 크롬 확장 `~/Documents/dec/` 역공학 → **무인증 읽기 REST API 발견 + 실호출 검증**(1,596키 × 5개 언어)
  - **P1 「구 표기 17키」의 수작업 실측치를 전건 재현** — zh `關注` 8·`追蹤` 3 · ja `友達` 4 · ko `월렛` 13
  - 권고: `fetch_xlt_registry.py` 신설 · **`validate_translation.py` 「등록값 유래 P1 강등」은 기각**(등록값이 원인인 P0(nbsp) 실사례 + 게이트 2번 근거 사례가 전부 기존 등록값의 실제 오류) · `--registry` 주석 전용
  - **부수 발견**: UIT 치환자 표기 불일치 15셀(위 P1)
- **배운 것**:
  - **읽기 경로 부재가 기술적 제약이 아니었다** — 확장이 쓰던 API가 그대로 열려 있었고, export 수동 요청·오탐 74건 재판정·3원 대조 수기가 전부 이 한 칸 때문에 파생됐다.
  - **대조를 자동화하니 사람이 못 보던 게 나왔다** — 57키를 눈으로 훑어서는 안 나올 치환자 표기 15셀 불일치가 딕셔너리 조인 한 번에 잡혔다.
  - **"API가 정규화한 것 아닌가"를 반드시 배제해야 했다** — `{{` 포함 키가 1,596키 중 1개 남아 있다는 사실이 언이스케이프 가설을 기각했다. 이 확인을 건너뛰었으면 잘못된 결론을 낼 뻔했다.
...
- **XLT 등록값 API**(2026-08-07 발견·실증 — 아직 스크립트 없음): `GET https://xlt-api.linecorp.com/downloadXLT/Dapp%20Portal/WEB%20BROWSER/{version}/{lang}/json` → flat `{키: 번역문}`(언어당 1건). 버전 목록은 `/xlt/info/versions/{service}/{device}?limit=30`. **무인증**(VPN 전제 추정). 1,596키 × 5개 언어. **export를 못 받은 세션에서도 이걸로 정본 대조가 가능하다** — 상세·재현 명령은 `reports/research/xlt_api_feasibility_2026-08-07.md`.
