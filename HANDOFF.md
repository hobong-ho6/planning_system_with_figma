...
- **⚠️ 등록된 키의 값을 고치기 전에 그 키의 다른 사용처를 확인한다**(2026-09-13 실측 · unifi-mini-v2) — 같은 팀 안에서도 요구가 갈린다: LV 한 명은 `mini_voucher_detail_max_benefit_rate`에서 `{0}%` **제거**를, 다른 한 명은 **유지**를 요청했다(Slack). 한쪽만 반영하면 다른 쪽 화면이 조용히 깨진다 → **값 변경 요청을 받으면 위키 Screen 표·다국어 표에서 그 키가 쓰이는 위치를 전부 훑고**, 요구가 갈리면 **키를 분리**한다. ⭐ **제거된 값은 `reports/gate/*.md`의 5개 언어 표에서 복원할 수 있다** — 레지스트리는 버전 컨테이너가 갱신돼 구값이 남지 않으므로, 게이트 리포트가 사실상 유일한 롤백 자산이다
...
- **Landpress — API로 되는 것과 안 되는 것을 구분한다**(2026-09-13 실측 2회 · `md/landpress.md` §10-3) — ⓐ **새 항목 생성은 된다**: `POST /items?locale={loc}` → 201 + 새 `postId`, **그 로케일이 primary**가 된다(실측: `oam_message_task_multi` beta에 15건 연속 생성 `217~231`, 전건 대조 통과). ⓑ **기존 항목에 로케일 행을 추가하는 경로는 없다** — 같은 호출이 기존 항목에 붙지 않고 **「고아 항목」을 새로 만든다**(body의 `postId` 무시). 2번째 언어부터는 **사용자가 CMS UI에서** 추가해야 하고, 그 뒤 API로 쓰기·공개한다. ⓒ 컬렉션 생성은 CMS UI. **권장 순서: Claude가 `ko_KR` 항목 생성 → 사용자가 로케일 추가 → Claude가 로케일별 PUT.** ⛔ 생성은 되돌리기 번거로우니 **사용자 지시가 있을 때만**, 대량이면 **1건 먼저 만들어 재조회 검증** 후 진행. ⚠️ **로케일 파라미터는 공개·CMS 둘 다 `?locale=`** — `?_locale=`은 **400이 아니라 조용히 무시**돼 primary가 돌아온다(2026-09-13 실측 정정 · `md/landpress.md` §9-1)
- **캐시 금지 실측**: 세션 #17 감사 도중 `UF_floating_jpyc_banner_title`이 **실제로 삭제**돼 1시간 만에 키 수가 2,131→2,130으로 바뀌었다. 원본 재조회는 형식이 아니다
- **⛔ 산출물은 만든 세션이 그 세션 안에서 커밋한다**(2026-09-13 신설 — 실측 누락) — 게이트 리포트 9건·`landpress/` 100파일이 **최대 9일간(09-04~09-13) 커밋되지 않고 로컬에만** 남아 있었다. 게이트 리포트는 **제거된 값을 복원할 수 있는 사실상 유일한 롤백 자산**인데(위 「등록된 키의 값을 고치기 전에」 항목), 그 PC가 아니면 존재하지 않는 상태였다. → **세션 종료 시 `git status`의 `??`를 반드시 확인**하고, 내가 만든 산출물이면 커밋한다. 남의 것으로 보이면 **발견 사실을 사용자에게 보고**한다(임의 커밋·삭제 금지). `reports/gate/`·`landpress/`는 `.gitignore` 대상이 아니다 — 미추적으로 남아 있으면 **누락이지 의도가 아니다**
