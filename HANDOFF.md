...
- **관련 링크**:
  ...
  - 용어집: **라이브 v4.5**(115 terms · `deprecated_terms` 13종 · `context` 4건) — 2026-08-07 API 조회로 재확인. 이력 `md/glossary-changelog.md` · IA 정본 `md/IA.md`
  - 기획자 가이드: **라이브 v20**(2026-08-07 게시 완료) — 산출물 `dropweb/web3_planning_v20.zip`(2.8M). **v20=「🧭 시스템 가이드」 탭 신설**(3계층 구조·흐름 지도 6종·상황별 라우팅 15행·4대 게이트·문서/스크립트 지도·확장 규칙) · v19=용어집 v4.5 · v18=용어집 이력 접이식 · v17=용어집 v4.4. ⚠ 최신본 판별은 파일명이 아니라 **푸터 내부 버전+용어집 임베드 버전+mtime**(`md/landpress.md` §5-1 0단계). `dropweb/`은 `.gitignore` 대상이라 **채팅 전달이 유일한 배포 경로**
...
> 마지막 갱신: 2026-08-07 (**세션 #17**) · 작업 PC **`AD03230205ui-iMac.local`** · branch `main` · **핸드오프 본문 커밋 `0229dd2`** — 그 뒤에 이 해시를 적는 **후속 커밋 1개가 HEAD**다(정상, 불일치 아님). 세션 작업분 `847e2de`·`a4139eb`·`1ba57bd`·`a48ddfb` — **전부 푸시 완료**

**진행 중 작업(WIP)**: **없음.** 작업 트리 clean.

**이번 세션(#17) — XLT 검증 체계 신설 + 시스템 전체 감사 + 가이드 v20 (4커밋, 전부 푸시):**

세션 #16이 "가능하다"만 확인하고 코드를 만들지 않은 것을 이어받아, **사용자 요청 2건**(① 번역 시점 유사 키 제안 ② 정기 용어집 검증)을 절차·스크립트로 구현하고 시스템 전체를 감사했다.

- **`847e2de` XLT 검증 체계** — 스크립트 3종(`fetch_xlt_registry.py` 조달·유사 키 검색 / `verify_xlt_glossary.py` 용어집 대조·업로드 엑셀 / `compare_wiki_xlt.py` 위키 대조) + **`md/xlt-verify.md` 신설** + `md/translate.md` **Step 2-1**(유사 키 제안 ⓐ신규/ⓑ재사용/ⓒ업데이트 3택) + `CLAUDE.md`·`.gitignore`. **검증 로직은 `validate_translation.py`를 그대로 재사용**(판정 기준 단일 출처).
- **`a4139eb` 전체 감사** — 3서비스 4,092키 × 용어집 v4.5. 자동 등급(P0 121·P1 4,173·P2 840)을 **전건 사람이 판정**해 실제 위반만 추림.
- **`1ba57bd` 교차 오염 탐지 매트릭스** — 20개 조합을 하나씩 주입해 실측, **15개만 탐지**. 사각지대 3종을 `md/check.md` **v3.4**에 패턴 사전으로 영구 기록.
- **`a48ddfb` 시각화 리포트** — `md/dropweb-guide.md` 규격 사이트(`xlt_audit_site/`, 단일 페이지 10섹션). 수치를 하드코딩하지 않고 **원본에서 재생성**한 덕에 리포트 오차 3건이 드러나 정정.

**⛔ 가장 큰 구조적 발견 — 서비스가 3개이고 키스페이스가 다르다.** `Dapp Portal`(1,596) · `Unifi`(2,130) · `Kaia Wallet`(366). 공통 335키 중 **151키(45%)가 값 상이** — 두 서비스가 **서로의 키를 구값 사본으로 보유**한다(`Dapp Portal`이 `UF_` 구값 87키, `Unifi`가 `unifi_promotion_` 구값 44키).

**✅ 세션 #16의 「UIT 치환자 불일치」는 원인이 규명됐다 — 문서 규칙이 맞았다.** `UF_`(UIT) 정본은 **`Unifi` 서비스**이고 거기서는 `{{0}}`가 279키로 `CLAUDE.md` 규칙과 일치한다. #16이 대조한 `Dapp Portal`은 **잘못된 서비스**였고 그 안의 `UF_` 32키가 `{0}` 사본이었다. #16의 가설 ⓐⓑⓒ 중 어느 것도 아니고 **조회 대상이 틀렸던 것**이다.

**✅ 「변경 5키 시스템 미갱신」도 해소 상태였다.** 시즌3 위키를 `Dapp Portal`과 대조하니 ⓔ 분기 10건이 **전부 Dapp Portal(정본)과 일치**했다. 남은 것은 `Unifi`의 구값 사본 정리 여부다.

**⚠️ 신규 확정 위반 — 키 이름 손상 3건이 최우선.** 문구 검증으로는 원리적으로 못 잡는 부류로, 키 이름 전수 점검을 따로 돌려 나왔다. `Dapp Portal`의 `"\xa0UF_home_jpyc_home_banner2"`(앞에 nbsp) · `Kaia Wallet`의 `"payment_history_\x08paymentstatus_title"`(제어문자 U+0008)·`"payment_history_?paymentstatus_title"`. 뒤 둘은 **ko↔en까지 뒤바뀐 중복 키**다. 상세는 `reports/audit/xlt_system_glossary_audit_2026-08-07.md`.

**가이드 v20 게시 완료**(사용자, 2026-08-07) — 「🧭 시스템 가이드」 탭 신설. 용어집 임베드는 v4.5 = 라이브 v4.5로 동일해 **용어집 탭은 갱신 대상이 아니었다**.
...
### 2026-08-07 — 세션 #15: 럭키볼 뽑았을때 OA 지급 시점 반영 (**HANDOFF 미갱신 — 커밋 로그 기준**)

- `f2993ca` 럭키볼 뽑았을때 OA 「2주 이내 지급 예정」 4개 언어 반영(캠페인 위키 `4479306980` **v151**) · `7077ff3` OA 3화면 「즉시 지급」 각주는 **그대로 둔다**(사용자 결정 종결)

### 2026-08-06 — 세션 #14: 시즌3 금액 정정 + 용어집 v4.5 + 가이드 v19 게시 (**HANDOFF 부분 갱신 — 커밋 로그 기준**)

- `e338cbf` 시즌3 UIT 2키 **15만엔 → 5만엔**(위키 v66, 엑셀 ALL v17·UIT v14) · `d3b7b89` 세션 #11·#12 결과 보완 · `f75c54e` 용어집 **v4.5**(`월렛`→`지갑`) · `a8710c7` 시즌3 변경 5키 + OA 2건 재번역(**위키 v76**) · `870fc33` 가이드 **v19 게시 완료 — P0 전건 해소**
- ⚠️ 이 세션은 관련 링크·다음 할 일·주요 결정은 갱신했으나 **「현재 상태」 블록과 세션 기록은 갱신하지 않았다** — 세션 #16이 커밋 로그로 복원했다.
...
- **핵심 산출물**(2026-08-06 기준): 시즌3 위키 **v76**·번역표 **57키** · 럭키볼 캠페인 75키 · `xlt/xlt_output_season3_{ALL,LV,UIT}_20260804.xlsx`(**ALL v19·LV v10·UIT v15**) · `oa/flex_OA_Reward_Confirm_*`(zip **v5**)·`oa/flex_OA_mission_complete_*`(zip **v4**) · `reports/gate/*.md` · `reports/audit/`(**`xlt_system_glossary_audit_2026-08-07.md` = XLT 전수 감사 정본**) · `reports/xlt/`(git 미추적 — 검증 리포트·수정 제안) · `reports/ia/` · `xlt_audit_site/`(감사 시각화) · **가이드 라이브 v20**
