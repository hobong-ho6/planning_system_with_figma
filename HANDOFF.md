...
| [unifi-mini-v2](handoff/projects/unifi-mini-v2.md) | `hogeun` | 위키 `4704515582` + 하위 **LPC `4727978725`** · **25화면** | 09-14 | 본문 **v178** · LPC **v41** — `최근 본 상품` **UIT→LV 이관**(키는 `UF_` 유지 → 「확정 필요 항목」에 **LV 회신 대기**) · 9-17 **이후 범위 표 2행 보강**(전수 확인) · K-Pick 1만원권 prod 전건 일치(beta 6건 뒤처짐) · 🔴 **한정해제 8개 파라미터 미결** → 파일 참조 |
